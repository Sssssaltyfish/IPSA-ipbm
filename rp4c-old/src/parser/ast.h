...
enum MatchType { MATCH_EXACT, MATCH_TERNARY, MATCH_LPM };

enum BinaryOp { OP_EQ, OP_NE, OP_LT, OP_GT, OP_LE, OP_GE };

static inline std::string to_string(MatchType type) {
    static const std::map<MatchType, std::string> m = {
        {MATCH_EXACT, "EXACT"}, {MATCH_TERNARY, "TERNARY"}, {MATCH_LPM, "LPM"}};
    return m.at(type);
}
...
struct FieldDef : public TreeNode {
    ...
    FieldDef(std::shared_ptr<FieldType> type_, std::string name_)
...
struct NextTableEntry : TreeNode {
    ...
    NextTableEntry(int key_, std::string val_)
        : key(key_), val(std::move(val_)) {}
};
...
struct HeaderDef : public TreeNode {
    ...
    HeaderDef(std::string name_, std::vector<FieldDef> fields_,
              NextTable next_table_)
        : name(std::move(name_)), fields(std::move(fields_)),
          next_table(std::move(next_table_)) {}
};
...
struct StructDef : public TreeNode {
    ...
    StructDef(std::string type_name_, std::vector<FieldDef> fields_,
              std::string var_name_)
        : type_name(std::move(type_name_)), fields(std::move(fields_)),
...
struct HeaderSequence : public TreeNode {
    ...
    HeaderSequence(std::vector<FieldDef> fields_, std::string name_)
        : fields(std::move(fields_)), name(std::move(name_)) {}
};
...
struct BinaryExpr : public Expr {
    ...
    BinaryExpr(BinaryOp op_, std::shared_ptr<Expr> lhs_,
...
struct MemberOf : public LValue {
    ...
    MemberOf(std::shared_ptr<Expr> recv_, std::string name_)
        : recv(std::move(recv_)), name(std::move(name_)) {}
};
...
struct CallExpr : public Expr {
    ...
    CallExpr(std::shared_ptr<Expr> caller_,
             std::vector<std::shared_ptr<Expr>> params_)
        : caller(std::move(caller_)), params(std::move(params_)) {}
};
...
struct AssignStmt : public Stmt {
    ...
    AssignStmt(std::shared_ptr<LValue> lhs_, std::shared_ptr<Expr> rhs_)
...
struct ActionDef : public TreeNode {
    ...
    ActionDef(std::string name_, std::vector<FieldDef> args_,
...
struct MatchKey : public TreeNode {
    ...
    MatchKey(std::shared_ptr<LValue> key_, MatchType match_type_)
        : key(std::move(key_)), match_type(match_type_) {}
};
...
struct TableDef : public TreeNode {
    ...
    TableDef(std::string name_, std::vector<MatchKey> match_keys_, int size_)
        : name(std::move(name_)), match_keys(std::move(match_keys_)),
          size(size_) {}
};
...
struct MatcherDefTable : public MatcherDef {
    ...
    MatcherDefTable(std::string table_name_)
...
struct MatcherDefCond : public MatcherDef {
    ...
    MatcherDefCond(std::vector<std::shared_ptr<Expr>> conds_)
...
struct StageDef : public TreeNode {
    ...
    StageDef(std::string name_, std::string parser_,
             std::shared_ptr<MatcherDef> matcher_,
             std::vector<std::pair<int, std::string>> executor_)
        : name(std::move(name_)), parser(std::move(parser_)),
          matcher(std::move(matcher_)), executor(std::move(executor_)) {}
};
...
struct ControlDef : public TreeNode {
    ...
    ControlDef(std::string name_, std::vector<StageDef> stages_)
        : name(std::move(name_)), stages(std::move(stages_)) {}
};
...
struct RP4AST : public TreeNode {
    ...
    RP4AST(std::vector<HeaderDef> headers_def_,
           std::vector<StructDef> struct_def_, HeaderSequence header_seq_,
           std::vector<ActionDef> actions_def_,
           std::vector<TableDef> tables_def_,
           std::vector<ControlDef> controls_def_)
        : headers_def(std::move(headers_def_)),
          structs_def(std::move(struct_def_)),
          header_seq(std::move(header_seq_)),
          actions_def(std::move(actions_def_)),
          tables_def(std::move(tables_def_)),
          controls_def(std::move(controls_def_)) {}
};
