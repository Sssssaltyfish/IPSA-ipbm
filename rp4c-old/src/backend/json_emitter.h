...
private:
    static json visit(const ast::TreeNode* node, Context& ctx) {
        ...
        if (node == nullptr) {
            ...
            int size;
            if (auto bit_type =
                    dynamic_cast<const ast::BitType*>(x->type.get())) {
                type = "bit";
                len = bit_type->len;
                size = -1;
            } else if (auto reg_type = dynamic_cast<const ast::RegisterType*>(
                           x->type.get())) {
                type = "register";
                auto bit_type =
                    dynamic_cast<const ast::BitType*>(reg_type->type.get());
                assert(bit_type);
                len = bit_type->len;
                size = reg_type->size;
            }
            ...
            assert(call_expr);
            auto caller =
                dynamic_cast<const ast::MemberOf*>(call_expr->caller.get());
            out["op"] = caller->name;
            out["args"] = json::array();
