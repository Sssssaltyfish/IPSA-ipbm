...
int main() {
    ...
    {
        uint8_t packet[] =
            "\x00\x00\x00\x00\x01\x02\x00\x00\x00\x00\x01\x03\x90\x00";
    ...
    {
        uint8_t packet[] =
            "\x00\x00\x00\x00\x01\x04\x00\x00\x00\x00\x01\x02\x90\x00";
        int len = sizeof(packet) - 1;
    ...
    {
        uint8_t packet[] =
            "\x00\x00\x00\x00\x99\x99\x00\x00\x00\x00\x01\x02\x90\x00";
        int len = sizeof(packet) - 1;
        Global::process_packet(packet, len, 0);
    ...
    // TEST IP FORWARDING
    {
        ...
        *eth_hdr = {.ether_dhost = {0x00, 0x00, 0x00, 0x00, 0x01, 0x01},
    ...
    }
    {
        ...
        uint8_t packet[BUFSIZ];
        auto eth_hdr = (ether_header*)packet;
        *eth_hdr = {.ether_dhost = {0x00, 0x00, 0x00, 0x00, 0x02, 0x01},
    ...
    {
        uint8_t packet[] =
            "\x00\x00\x00\x00\x01\x03\x00\x00\x00\x00\x01\x02\x90\x00";
        int len = sizeof(packet) - 1;
        Global::process_packet(packet, len, 0);
    ...
    {
        uint8_t packet[] =
            "\x00\x00\x00\x00\x01\x03\x00\x00\x00\x00\x01\x05\x90\x00";
        int len = sizeof(packet) - 1;
