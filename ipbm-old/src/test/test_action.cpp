...
int main() {
    ...
    action.ops = {
        ...
        Primitive::cksum(FieldSpec(1, 0, sizeof(iphdr)),
                         ...
                         HASH_CSUM16)};
    std::cout << std::hex;
    std::cout << "Prev: ttl=" << (int)ip_packet->ttl
              << " cksum=" << (int)ip_packet->check << std::endl;
    uint8_t prev_ttl = ip_packet->ttl;
    uint16_t prev_cksum = ip_packet->check;

    // Execute
    int ACT_ID = 0;
    Executor executor = Executor(ACT_ID, Processor::PROC_ID_END, ACT_ID,
    ...
    std::cout << "Curr: ttl=" << (int)ip_packet->ttl
              << " cksum=" << (int)ip_packet->check << std::endl;
