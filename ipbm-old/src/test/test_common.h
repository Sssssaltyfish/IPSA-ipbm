...
class TestRouter {
public:
    static Executor build_executor_forward(int act_none_id, int act_fwd_id,
                                           int hdr_ip_id, int hdr_ether_id) {
        Executor executor(
            act_fwd_id, Processor::PROC_ID_END, act_none_id,
    ...
    static Parser build_parser_ipv4(int hdr_ether_id, int hdr_ip_id) {
        ...
        ParserHeader hdr_ether(
            hdr_ether_id, sizeof(ether_header),
            offsetof(ether_header, ether_type), sizeof(uint16_t),
    ...
    static void build_arp_table(int tab_arp_id, int hdr_ip_id, int act_none_id,
                                int act_fwd_id) {
        int arp_val_len = sizeof(ether_addr) + sizeof(uint32_t);
        Processor proc;
        uint8_t tag = 0;
        FlowTable tab_arp(
            tag, arp_val_len, glb.logic_tables[0], MEM_SRAM, MATCH_EXACT,
