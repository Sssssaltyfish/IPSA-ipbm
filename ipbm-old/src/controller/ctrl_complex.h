...
public:
    ...
    CtrlFwd(std::shared_ptr<grpc::Channel> channel)
        : api_client(std::move(channel)) {}

    void basic_forward() {
        RC rc;
        // ----- BUILD VLAN TABLE -----
        std::vector<std::pair<int, uint32_t>> vlan_table = {
            {0, 0x0f}, {1, 0x0f}, {2, 0x0f}, {3, 0x0f}, {4, 0x10},
        };
        for (auto& entry : vlan_table) {
            ...
            src_flow.action_data = {
                std::string((char*)&src_port, sizeof(uint32_t))};

            api::FlowEntry dst_flow;
            dst_flow.key = std::string((char*)&iface, sizeof(int));
            dst_flow.action_id = -1;
            dst_flow.action_data = {
                std::string((char*)&dst_port_mask, sizeof(uint32_t))};

            rc = api_client.mod_flow_entry(TAB_ID_VLAN_GROUP, src_flow,
                                           dst_flow);
            if (rc) {
                LOG(ERROR) << "Error code " << rc;
            }
        }
        ...
        // ----- BUILD FLOW TABLE -----
        {
            ...
            rc = api_client.set_flow_table(
                TAB_MAC_ID, 1, MEM_SRAM, MATCH_EXACT,
        ...
        }
        {
            ...
            rc = api_client.set_flow_table(
                ...
                {FieldSpec(HDR_IP_ID, offsetof(iphdr, daddr),
                           sizeof(in_addr_t))},
        ...
        rc = api_client.set_executor(PROC_ETHER_ID, ACT_MAC_FWD_ID,
                                     Processor::PROC_ID_END, ACT_MAC_BCAST,
                                     PROC_IP_ID, mac_fwd_args);
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
        ...
        rc = api_client.set_executor(PROC_IP_ID, ACT_IP_FWD_ID,
                                     Processor::PROC_ID_END, ACT_NONE_ID,
                                     Processor::PROC_ID_END, ip_fwd_args);
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }

        // ----- BUILD PROCESSOR -----
        rc = api_client.set_processor(
            PROC_ETHER_ID, Condition::is_valid(HDR_ETHER_ID),
            NextStage::table(TAB_MAC_ID), NextStage::proc(PROC_IP_ID));
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
        rc =
            api_client.set_processor(PROC_IP_ID, Condition::is_valid(HDR_IP_ID),
        ...
        }
        {
            ...
            flow.key = std::string((char*)&ip, sizeof(in_addr_t));
            flow.action_id = ACT_IP_FWD_ID;
            flow.action_data = {std::string((char*)mac2, sizeof(ether_addr)),
                                std::string((char*)&port, sizeof(uint32_t))};
            rc = api_client.add_flow_entry(TAB_ARP_ID, flow);
            if (rc) {
                LOG(ERROR) << "Error code " << rc;
    ...
    void add_tunnel() {
        ...
        rc = api_client.add_parser_header({HDR_ETHER_ID}, tunnel_tag_s,
                                          hdr_tunnel);
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
        ...
        // ----- BUILD FLOW TABLE -----
        rc = api_client.set_flow_table(
            TAB_TUNNEL_ID, 2, MEM_SRAM, MATCH_EXACT,
        ...
        rc = api_client.set_executor(PROC_TUNNEL_ID, ACT_TUNNEL_FWD_ID,
                                     Processor::PROC_ID_END, ACT_NONE_ID,
                                     Processor::PROC_ID_END, args_tunnel);
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
        ...
        rc = api_client.set_processor(
            PROC_TUNNEL_ID, Condition::is_valid(HDR_TUNNEL_ID),
    ...
    void add_counter() {
        ...
        // ----- BUILD FLOW TABLE -----
        rc = api_client.set_flow_table(
            TAB_COUNTER_ID, 3, MEM_SRAM, MATCH_EXACT,
        ...
        rc = api_client.set_executor(PROC_IP_ID, ACT_IP_FWD_ID, PROC_COUNTER_ID,
                                     ACT_NONE_ID, PROC_COUNTER_ID, args_ip_fwd);
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
        rc = api_client.set_executor(PROC_COUNTER_ID, ACT_NONE_ID,
                                     Processor::PROC_ID_END, ACT_NONE_ID,
        ...
        // ----- BUILD PROCESSOR -----
        rc = api_client.set_processor(PROC_COUNTER_ID,
                                      Condition::is_valid(HDR_IP_ID),
                                      NextStage::table(TAB_COUNTER_ID),
                                      NextStage::proc(Processor::PROC_ID_END));
        if (rc) {
            LOG(ERROR) << "Error code " << rc;
        }
