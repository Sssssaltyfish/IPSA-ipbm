...
Global::Global() {
    ...
    std::vector<TableSpec> sram_spec = {
        {0, 10, 10, 257}, {1, 20, 20, 257}, {2, 24, 24, 257}, {3, 32, 32, 257},
        {0, 48, 48, 257}, {1, 64, 64, 257}, {2, 80, 80, 257}, {3, 96, 96, 257},
    };
    for (auto& spec : sram_spec) {
        glb.logic_tables.emplace_back(
            std::make_shared<LogicSram>(spec.physical_table_id, spec.key_len,
                                        spec.val_len, spec.num_entry));
    }
    // Init logical TCAM
    ...
    for (auto& spec : tcam_spec) {
        glb.logic_tables.emplace_back(
            std::make_shared<LogicTcam>(spec.physical_table_id, spec.key_len,
                                        spec.val_len, spec.num_entry));
    }
}
    ...
    for (struct ifaddrs* ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        for (int i = 0; i < glb.num_if; i++) {
            if (ifa->ifa_addr->sa_family == AF_PACKET &&
                ...
                // found
                ether_addr mac;
                memcpy(&mac, ((struct sockaddr_ll*)ifa->ifa_addr)->sll_addr,
                       sizeof(ether_addr));
                LOG(INFO) << "MAC address of interface " + glb.if_names[i] +
                                 " is " + mac2str(mac.ether_addr_octet);
