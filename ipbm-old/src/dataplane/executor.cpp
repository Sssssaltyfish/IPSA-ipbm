...
int Executor::execute(Buffer match_val) {
    ...
    for (auto& op : glb.actions[action_id].ops) {
        ...
        case OP_CKSUM: {
            ...
            Buffer cksum_field =
                glb.hdrs[args.cksum_field.hdr_id].data + args.cksum_field.start;
            Buffer dst_field =
                glb.hdrs[args.dst_field.hdr_id].data + args.dst_field.start;
            *(uint16_t*)dst_field = 0;
            *(uint16_t*)dst_field =
                calc_cksum(cksum_field, args.cksum_field.len, args.hash_type);
            break;
        }
        case OP_COPY_FIELD: {
        ...
        case OP_ADD: {
            ...
            Buffer val_buf =
                glb.hdrs[args.field.hdr_id].data + args.field.start;
            if (args.field.len == 1) {
