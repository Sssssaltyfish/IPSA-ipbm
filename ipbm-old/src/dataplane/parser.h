...
struct NextParserHeader {
    ...
    NextParserHeader(std::string tag, int hdr_id)
        : tag(std::move(tag)), hdr_id(hdr_id) {}
};
...
struct ParserHeader {
    ...
    std::vector<NextParserHeader>
        next_table; // type id -> parse node idx, accept by -1

    ParserHeader() = default;

    ParserHeader(int hdrId, int hdrLen, int nextHdrTypeStart,
                 int nextHdrTypeLen, std::vector<NextParserHeader> nextTable)
