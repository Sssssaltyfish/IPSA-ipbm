...
class ModServiceImpl final : public rp4::ModService::Service {
    grpc::Status handleMatchTable(grpc::ServerContext* context,
                                  const rp4::FlowEntryModRequest* request,
                                  rp4::ReturnCodeResponse* response) override {
        RC rc;
        if (request->mod_type() == rp4::ADD) {
            auto src_flow = load_flow_entry(request->srcflow());
            rc = add_flow_entry(MEM_SRAM, request->flow_table_id(),
                                src_flow); // TODO use mem type
            ...
            rc = mod_flow_entry(MEM_SRAM, request->flow_table_id(), src_flow,
                                dst_flow);
        } else {
    ...
    grpc::Status getFlowEntry(grpc::ServerContext* context,
                              const rp4::FlowEntryRequest* request,
                              rp4::FlowEntry* response) override {
        auto query_flow = load_flow_entry(request->flow_entry());
        auto val = get_flow_entry_action_data(request->flow_table_id(),
                                              query_flow.key);
        response->add_action_data(val);
        return grpc::Status::OK;
    }

    grpc::Status addParserInfo(grpc::ServerContext* context,
                               const rp4::ModParserInfo* request,
                               rp4::ReturnCodeResponse* response) override {
        auto& pb_hdr = request->header();
        std::vector<NextParserHeader> next_table;
        for (auto& pb_next_hdr : pb_hdr.next_headers()) {
            NextParserHeader next_hdr(pb_next_hdr.tag(),
                                      pb_next_hdr.header_id());
            next_table.push_back(next_hdr);
        }
        ParserHeader hdr(pb_hdr.header_id(), pb_hdr.header_length(),
                         pb_hdr.next_header_type_internal_offset(),
    ...
    grpc::Status addMatcherInfo(grpc::ServerContext* context,
                                const rp4::ModMatcherInfo* request,
                                rp4::ReturnCodeResponse* response) override {
        response->set_return_code(0);
        return grpc::Status::OK;
    }

    grpc::Status addActionInfo(grpc::ServerContext* context,
                               const rp4::AddActionInfo* request,
                               rp4::ReturnCodeResponse* response) override {
        Action action;
    ...
    grpc::Status addExecuterInfo(grpc::ServerContext* context,
                                 const rp4::AddExecuterInfo* request,
                                 rp4::ReturnCodeResponse* response) override {
        std::vector<RuntimeArgSpec> args_spec;
        for (auto& pb_arg_spec : request->args_spec()) {
            RuntimeArgSpec arg_spec(pb_arg_spec.internal_offset(),
                                    pb_arg_spec.field_length());
            args_spec.push_back(arg_spec);
        }
        set_executor(request->processor_id(), request->hit_action(),
                     request->hit_next_proc(), request->miss_action(),
                     request->miss_next_proc(), args_spec);
        response->set_return_code(0);
        return grpc::Status::OK;
    }

    grpc::Status addFlowTable(grpc::ServerContext* context,
                              const rp4::AddFlowTableInfo* request,
                              rp4::ReturnCodeResponse* response) override {
        auto& pb_flow_table = request->flow_table();
        std::vector<FieldSpec> key_fields;
        for (auto& pb_key_field : pb_flow_table.key_fields()) {
            FieldSpec key_field(pb_key_field.header_id(),
                                pb_key_field.internal_offset(),
                                pb_key_field.field_length());
            key_fields.push_back(key_field);
        }
        RC rc = set_flow_table(pb_flow_table.flow_table_id(),
                               pb_flow_table.resided_logical_table_id(),
                               load_mem_type(pb_flow_table.mem_type()),
                               load_match_type(pb_flow_table.match_type()),
                               key_fields, pb_flow_table.value_length(),
                               pb_flow_table.is_counter());
        response->set_return_code(rc);
        return grpc::Status::OK;
    }

    grpc::Status addProcessor(grpc::ServerContext* context,
                              const rp4::AddProcessorRequest* request,
                              rp4::ReturnCodeResponse* response) override {
        auto true_next = load_next_stage(request->true_next());
        auto false_next = load_next_stage(request->false_next());
        set_processor(request->proc_id(),
                      Condition::is_valid(request->valid_hdr_id()), true_next,
                      false_next);
        response->set_return_code(0);
        return grpc::Status::OK;
    }

    grpc::Status addMetaData(grpc::ServerContext* context,
                             const rp4::AddMetaDataRequest* request,
                             rp4::ReturnCodeResponse* response) override {
        response->set_return_code(0);
        return grpc::Status::OK;
    }

    grpc::Status requestMemoryInfo(grpc::ServerContext* context,
                                   const rp4::MemoryInfoRequest* request,
                                   rp4::MemoryInfo* response) override {

        return grpc::Status::OK;
    }

    grpc::Status initSRAMandTCAM(grpc::ServerContext* context,
                                 const rp4::MemoryInfo* request,
                                 rp4::ReturnCodeResponse* response) override {
        response->set_return_code(0);
        return grpc::Status::OK;
    }

    grpc::Status initParser(grpc::ServerContext* context,
                            const rp4::InitParserInfo* request,
                            rp4::ReturnCodeResponse* response) override {
        response->set_return_code(0);
        return grpc::Status::OK;
    }
...
private:
    ...
    static FieldSpec
    parse_param_field(const rp4::Primitive::Parameter& pb_param) {
