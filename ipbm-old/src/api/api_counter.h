...
public:
    ...
    static const int PROC_COUNTER_ID = 1;
    ...
    static const int TAB_COUNTER_ID = 1;
    ...
    static void build() {
        ...
        assert(add_parser_header({HDR_ETHER_ID},
                                 {std::string((char*)&ip_tag, sizeof(ip_tag))},
                                 hdr_ip) == 0);
        ...
        set_flow_table(
            TAB_COUNTER_ID, 1, MEM_SRAM, MATCH_EXACT,
        ...
        set_executor(PROC_FWD_ID, ACT_FWD_ID, PROC_COUNTER_ID, ACT_NONE_ID,
                     PROC_COUNTER_ID, runtime_args_spec);
