...
enum ModType { MOD_INSERT, MOD_DELETE, MOD_UPDATE };
...
RC mod_flow_entry(MemType mem_type, int table_id, const FlowEntry& src_flow,
                  const FlowEntry& dst_flow) {
    if (table_id == TAB_ID_VLAN_GROUP) {
        // special case
        assert(src_flow.key.size() == sizeof(int));
        assert(dst_flow.action_data.size() == 1 &&
               dst_flow.action_data[0].size() == sizeof(uint32_t));
...
RC set_flow_table(int flow_table_id, int logic_table_id, MemType mem_type,
    ...
    glb.flow_tables[flow_table_id] =
        FlowTable(flow_table_id, val_len, glb.logic_tables[logic_table_id],
                  mem_type, match_type, key_fields, is_counter);
    return 0;
}

RC set_executor(int proc_id, int hit_action, int hit_next_proc, int miss_action,
                int miss_next_proc,
                std::vector<RuntimeArgSpec> runtime_args_spec) {
    auto& proc = glb.processors[proc_id];
    proc.executor = Executor(hit_action, hit_next_proc, miss_action,
                             miss_next_proc, std::move(runtime_args_spec));
    return 0;
}
...
RC set_processor(int proc_id, const Condition& cond, const NextStage& true_next,
                 const NextStage& false_next) {
    auto& proc = glb.processors[proc_id];
    proc.cond = cond;
