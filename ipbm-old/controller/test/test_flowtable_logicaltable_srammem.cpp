...
int main() {
    ...
    auto ft = new FlowTable("mac_learning", MatchType::EXACT, MemType::MEM_SRAM,
                            32, 64, 1, 100);
    FlowTablePureInfo a = SRAM_handler->addPureFlowTable(ft);
    std::cout << a.flow_table_id << " " << a.flow_table_name << " "
              << a.residedLogicTableId << std::endl;
    SRAM_handler->addFlowEntry(1, "mac_learning");
    SRAM_handler->printSRAMInfo();

#ifdef SRAM
    auto ft1 =
        new FlowTable(1, MatchType::EXACT, MemType::MEM_SRAM, 1, 32, 64, 2);
    auto ft2 =
        new FlowTable(2, MatchType::EXACT, MemType::MEM_SRAM, 1, 48, 64, 1);
    auto ft3 =
        new FlowTable(3, MatchType::EXACT, MemType::MEM_SRAM, 2, 32, 32, 1);
    auto ft4 =
        new FlowTable(4, MatchType::EXACT, MemType::MEM_SRAM, 3, 64, 32, 1);
    auto ft5 =
        new FlowTable(5, MatchType::EXACT, MemType::MEM_SRAM, 3, 48, 64, 2);
    ...
    for (auto& ele : sram_mem_pool) {
        std::cout << "**************** SRAM no: " << ele.first
                  << " ****************" << std::endl;
        for (auto& it : ele.second) {
            std::cout << "logical table ID: " << it->getLogicalTableId()
                      << std::endl;
            auto flow_tables = it->getInsideFlowTables();
            for (auto& ft_it : flow_tables) {
                std::cout << "\t\tflow table ID: " << ft_it->getFlowTableId()
                          << std::endl;
            }
        }
    }
