...
int main() {
    ...
    matcher_config->initFromJson("../config/matcher.json", parser, mem_config,
                                 processor);
    ...
    //
    //    auto ft = new FlowTable("mac_learning", MatchType::EXACT,
    //    MemType::MEM_SRAM, 32, 64, 1, 100); FlowTablePureInfo a =
    //    SRAM_handler->addPureFlowTable(ft); std::cout << "New flow table " <<
    //    a.flow_table_name << "'s id is " << a.flow_table_id << std::endl;
    //    SRAM_handler->printSRAMInfo();
    ...
    //
    //    std::string match_fields[2] = {"Ethernet dmac exact", "VLAN vid
    //    exact"}; char match_type[50]; auto *head = new MatchFieldNode();
    //    MatchFieldNode *cur = head;
    //    for(auto & match_field : match_fields){
    //        auto mfn = new MatchFieldNode();
    //        sscanf(match_field.c_str(), "%s %s %s", mfn->header_name,
    //        mfn->field_name, match_type); if(std::strcmp(match_type, "exact")
    //        == 0) {
