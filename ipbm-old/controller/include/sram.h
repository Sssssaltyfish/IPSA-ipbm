...
public:
    ...
    //             : capacity(cap), free_capacity(cap),
    //             logical_tables(std::move(logicalTables)) {};
...
void SRAM::setSramId(int sramId) { this->sram_id = sramId; }

int SRAM::getSramId() { return this->sram_id; }

void SRAM::setLogicalTables(std::vector<int> lts) {
    this->logical_table_ids = lts;
}

std::vector<int> SRAM::getLogicalTables() { return this->logical_table_ids; }

void SRAM::addLogicalTable(int logical_table_id) {
    this->logical_table_ids.push_back(logical_table_id);
}

int SRAM::getCapacity() { return this->capacity; }

int SRAM::getFreeCapacity() { return this->free_capacity; }
//
...
//}
//
// std::unordered_map<int32_t, std::vector<FlowTable*>>
// SRAM::getFlowTablesWithCluster() {
//    std::vector<int> fts;
//    for(auto & it : this->logical_tables){
//        for (auto & its : it->getInsideFlowTables()){
//            fts.push_back(its->getFlowTableId());
