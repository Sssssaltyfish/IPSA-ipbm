...
public:
    FlowTable()
        : flow_table_id(-1), table_match_type(MatchType::NONE),
          mem_type(MemType::NONE), resided_logic_table_id(-1), key_length(-1),
          value_length(-1), action_data_num(-1), current_entry_num(0){};

    FlowTable(int32_t flowTableId, std::string flowTableName,
              MatchType tableMatchType, MemType memType,
              int32_t residedLogicTableId, int32_t keyLength,
              int32_t valueLength, int32_t actionDataNum,
              int32_t possibleEntryNum);

    FlowTable(std::string flowTableName, MatchType tableMatchType,
              MemType memType, int32_t keyLength, int32_t valueLength,
              int32_t actionDataNum, int32_t possibleEntryNum);
...
FlowTable::FlowTable(int32_t flowTableId, std::string flowTableName,
                     MatchType tableMatchType, MemType memType,
                     int32_t residedLogicTableId, int32_t keyLength,
                     int32_t valueLength, int32_t actionDataNum,
                     int32_t possibleEntryNum)
    : flow_table_id(flowTableId), table_match_type(tableMatchType),
      mem_type(memType), resided_logic_table_id(residedLogicTableId),
      key_length(keyLength), value_length(valueLength),
      action_data_num(actionDataNum), possible_entry_num(possibleEntryNum),
      current_entry_num(0) {
    // std::strcpy(flow_table_name, flowTableName);
    flow_table_name = flowTableName;
}

int32_t FlowTable::getFlowTableId() const { return flow_table_id; }

void FlowTable::setFlowTableId(int32_t flowTableId) {
    flow_table_id = flowTableId;
}

MatchType FlowTable::getTableMatchType() const { return table_match_type; }

void FlowTable::setTableMatchType(MatchType tableMatchType) {
    table_match_type = tableMatchType;
}

MemType FlowTable::getMemType() const { return mem_type; }

void FlowTable::setMemType(MemType memType) { mem_type = memType; }
...
int32_t FlowTable::getKeyLength() const { return key_length; }

void FlowTable::setKeyLength(int32_t keyLength) { key_length = keyLength; }

int32_t FlowTable::getValueLength() const { return value_length; }

void FlowTable::setValueLength(int32_t valueLength) {
    value_length = valueLength;
}

int32_t FlowTable::getActionDataNum() const { return action_data_num; }
...
void FlowTable::setPossibleEntryNum(int num) { this->possible_entry_num = num; }

int32_t FlowTable::getCurrentEntryNum() const { return current_entry_num; }
...
std::string FlowTable::getFlowTableName() { return this->flow_table_name; }
...
FlowTable::FlowTable(std::string flowTableName, MatchType tableMatchType,
                     MemType memType, int32_t keyLength, int32_t valueLength,
                     int32_t actionDataNum, int32_t possibleEntryNum) {
