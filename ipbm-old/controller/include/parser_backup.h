...
struct Header {
    ...
    void setHeaderName(char* headerName) { header_name = headerName; }
};
...
void Parser::setHeaders(const std::vector<Header*>& v_headers) {
    for (auto it : v_headers) {
        header_name_id_map.insert(
            std::make_pair(it->header_name, it->header_id));
...
void Parser::addHeaderBeforeRuntime(Header* header) {
    header_name_id_map.insert(
        std::make_pair(header->header_name, header->header_id));
...
void Parser::addNextHeaders(
    const std::vector<NextHeader*>& next_header_cluster) {
...
void Parser::addNextHeader(NextHeader* nextHeader) {
    next_header_map.insert(
        std::make_pair(nextHeader->next_header_id, nextHeader));
}
...
void Parser::setNextHeaderMap(
    std::unordered_map<int, NextHeader*> nextHeaderMap) {
...
void Parser::printHeaderInfo() {
    for (auto it : header_map) {
        ...
        std::cout << "********* " << header->header_id << ":"
                  << header->header_name << " *********" << std::endl;
        std::cout << "next_header_type_length: "
                  << header->next_header_type_length << std::endl;
        std::cout << "next_header_type_internal_offset: "
                  << header->next_header_type_internal_offset << std::endl;
        ...
        for (auto ele : header->fields) {
            std::cout << "\t" << field_map[ele]->field_id << " "
                      << field_map[ele]->field_name << " "
                      << field_map[ele]->field_length << std::endl;
        }
...
void Parser::addField(Field* field) {
    ...
    field_name_id_map.insert(
        std::make_pair(field->field_name, field->field_id));
}
...
//
//         if (s[0] == 'h') {
//             sscanf(s, "h %d %d %d %s", &header_id, &field_num,
//             &header_length, header_name); auto h = new Header(); h->header_id
//             = header_id; h->setHeaderName(header_name); h->header_length =
//             header_length; headers.push_back(h);
//             header_map.insert(std::make_pair(header_id, h));
//         } else if (s[0] == 'f'){
//             sscanf(s, "f %d.%d %d %d %s", &header_id, &field_id,
//             &field_length, &field_internal_offset, field_name); auto f = new
//             Field(); f->field_length = field_length; f->field_id = field_id;
...
//            &next_header_type_internal_offset, &next_header_type_length); if
...
void Parser::initializeFromPlainFile(const std::string& filename) {
    ...
    while (in.getline(s, 255)) {
        ...
        if (s[0] == 'h') {
            ...
            header_name_id_map.insert(
                std::make_pair(h->header_name, h->header_id));
            ...
            field_name_id_map.insert(
                std::make_pair(f->field_name, f->field_id));
...
void Parser::initializeFromJson(const std::string& json_filename) {
    ...
    for (auto it = d.MemberBegin(); it != d.MemberEnd(); ++it) {
        ...
        h->next_header_type_internal_offset =
            it->value["next_header_type_internal_offset"].GetInt();
        h->next_header_type_length =
            it->value["next_header_type_length"].GetInt();

        this->header_map.insert(std::make_pair(h->header_id, h));
        this->header_name_id_map.insert(
            std::make_pair(h->header_name, h->header_id));

        for (auto ele = it->value["fields"].MemberBegin();
             ele != it->value["fields"].MemberEnd(); ++ele) {
            auto f = new Field();
            f->field_name = ele->value["field_name"].GetString();
            f->field_id = cur_field_idx++;
            f->field_length = ele->value["field_length"].GetInt();
            f->field_internal_offset =
                ele->value["field_internal_offset"].GetInt();
        ...
        for (auto ele = it->value["next_headers"].MemberBegin();
             ele != it->value["next_headers"].MemberEnd(); ++ele) {
            auto nh = new NextHeader();
            nh->next_header_name = ele->value["header_name"].GetString();
            nh->next_header_tag = ele->value["header_tag"].GetString();

            next_header_name_map.insert(
                std::make_pair(nh->next_header_name, nh));
            h->next_header_names.push_back(
                nh->next_header_name); // TODO: transfer name to id
        }
    }
    ...
    for (auto it : header_map) {
        ...
        for (auto ele : header->next_header_names) {
            auto next_header_id =
                this->getHeaderIdByName(ele); // transfer name to ID
            header->next_header_ids.push_back(next_header_id);
            header_map[next_header_id]->father_header_ids.push_back(
                header->header_id);
        }
    }
}
...
//         h->next_header_type_internal_offset =
//         it["next_header_type_internal_offset"].asInt();
//         h->next_header_type_length = it["next_header_type_length"].asInt();
//
//         this->header_map.insert(std::make_pair(h->header_id, h));
//         this->header_name_id_map.insert(std::make_pair(h->header_name,
//         h->header_id));
//
...
//             f->field_internal_offset =
//             field["field_internal_offset"].asInt();
//
//             this->field_map.insert(std::make_pair(f->field_id, f));
//             field_name_id_map.insert(std::make_pair(f->field_name,
//             f->field_id)); h->fields.push_back(f->field_id);
//         }
//
//         //TODO:
//         // 1. add header_tag/father_ids
...
//
//             next_header_name_map.insert(std::make_pair(nh->next_header_name,
//             nh)); h->next_header_names.push_back(nh->next_header_name);//
//             TODO: transfer name to id
//         }
//     }
//
//     // initiate next_header_id using header name
...
//         auto next_header_id = this->getHeaderIdByName(it.first);
//         it.second->next_header_id = next_header_id;
//         this->next_header_map.insert(std::make_pair(next_header_id,
//         it.second));
