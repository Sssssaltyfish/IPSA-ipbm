...
class Parser {
public:
    Parser() : cur_header_idx(0), cur_field_idx(0){};
    ...
    void addHeaderAtRuntime(Header* header, const std::vector<Field*>& fields,
                            const std::vector<NextHeader*>& next_headers);
    ...
    std::unordered_map<int, Header*> getHeaderMap();
    std::unordered_map<int, Field*> getFieldMap();

    int getMaxFieldNo();

    int getHeaderIdByName(std::string header_name);
    int getFieldIdByName(std::string field_name);
...
void Parser::addHeaderAtRuntime(Header* header,
                                ...
                                const std::vector<NextHeader*>& next_headers) {
    header->header_id = cur_header_idx;
    cur_header_idx++;
    header_name_id_map.insert(
        std::make_pair(header->header_name, header->header_id));
    header_map.insert(std::make_pair(header->header_id, header));
    ...
    for (auto field : fields) {
        field->field_id = cur_field_idx;
        header->fields.push_back(cur_field_idx);
        cur_field_idx++;
        field_map.insert(std::make_pair(field->field_id, field));
    ...
    for (auto it : next_headers) {
        if (it->next_header_id != -1) {
            header->next_header_ids.push_back(it->next_header_id);
            next_header_map.insert(std::make_pair(it->next_header_id, it));
            header_map[it->next_header_id]->father_header_ids.push_back(
                header->header_id);
        }
    }
...
int Parser::getFieldIdByName(std::string field_name) {
...
std::unordered_map<int, Header*> Parser::getHeaderMap() {
    return this->header_map;
}

std::unordered_map<int, Field*> Parser::getFieldMap() {
 ...
 * not as useful as initializeFromJson
 * @param filename
 */
...
/**
 * see the json file in '../config/header.json'
 * @param json_filename
 */
