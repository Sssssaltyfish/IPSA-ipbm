...
public:
    MemConfig()
        : cur_sram_idx(0), cur_tcam_idx(0), cur_logical_table_idx(0),
    ...
    FlowTable* addPureFlowTable(const std::string& flow_table_name,
                                const std::string& matchType, int key_length,
                                int value_length, int action_data_num,
                                int possible_entry_num);
    ...
    LogicalTable* calLogicalTableByFlowTable(MemType mem_type, int key_length,
                                             int value_length,
                                             int possible_entry_num);
    ...
    void setLogicalTableMap(
        const std::unordered_map<int, LogicalTable*>& logicalTableMap);
    const std::unordered_map<int, FlowTable*>& getFlowTableMap() const;
    void
    setFlowTableMap(const std::unordered_map<int, FlowTable*>& flowTableMap);
    const std::unordered_map<std::string, int>& getFlowTableNameIdMap() const;
    void setFlowTableNameIdMap(
        const std::unordered_map<std::string, int>& flowTableNameIdMap);
...
void MemConfig::initSRAMFromJson(const std::string& json_filename) {
    ...
    for (auto it = d["srams"].MemberBegin(); it != d["srams"].MemberEnd();
        ...
        for (auto ele = it->value.MemberBegin(); ele != it->value.MemberEnd();
             ++ele) {
            auto lt = new LogicalTable(cur_logical_table_idx, MemType::MEM_SRAM,
                                       cur_sram_idx,
...
void MemConfig::initTCAMFromJson(const std::string& json_filename) {
    ...
    for (auto it = d["tcams"].MemberBegin(); it != d["tcams"].MemberEnd();
         ++it) {
        auto lt =
            new LogicalTable(cur_logical_table_idx, MemType::MEM_TCAM,
                             cur_tcam_idx++, it->value["entry_length"].GetInt(),
                             it->value["entry_num"].GetInt());
        logical_table_map.insert(std::make_pair(lt->getLogicalTableId(), lt));
        tcam_map[lt->getResidedHardwareId()]->setLogicalTableId(
            cur_logical_table_idx);
        cur_logical_table_idx++;
    }
}
...
LogicalTable* MemConfig::calLogicalTableByFlowTable(MemType mem_type,
                                                    int key_length,
                                                    int value_length,
                                                    int possible_entry_num) {
    ...
    if (mem_type == MemType::MEM_TCAM) {
        ...
        for (auto& it : logical_table_map) {
            ...
            if (lt->getCurrentCapacity() >= possible_entry_num &&
                ...
                (supportedMaxLength - key_length - value_length) <=
...
void MemConfig::initFlowTableFromJson(const std::string& json_filename) {
    ...
    for (auto it = d.MemberBegin(); it != d.MemberEnd(); ++it) {
        ...
        LogicalTable* lt = this->calLogicalTableByFlowTable(
            getMemType(mem_type), key_length, value_length, possible_entry_num);

        FlowTable* ft = new FlowTable(
            cur_flow_table_idx, flow_table_name, getMatchType(match_type),
            getMemType(mem_type), lt->getLogicalTableId(), key_length,
            value_length, action_data_num, possible_entry_num);

        lt->addInsideFlowTable(cur_flow_table_idx);
        this->flow_table_map.insert(std::make_pair(ft->getFlowTableId(), ft));
        flow_table_name_id_map.insert(
            std::make_pair(ft->getFlowTableName(), ft->getFlowTableId()));
        cur_flow_table_idx++;
    }
}

FlowTable* MemConfig::addPureFlowTable(const std::string& flow_table_name,
    ...
    auto ft = new FlowTable(flow_table_name, match_type, mem_type, key_length,
                            value_length, action_data_num, possible_entry_num);
...
void MemConfig::addFlowTable(FlowTable* ft) {
    ...
    LogicalTable* lt = this->calLogicalTableByFlowTable(
        ft->getMemType(), ft->getKeyLength(), ft->getValueLength(),
...
int MemConfig::getTcamNum() const { return tcam_num; }

void MemConfig::setTcamNum(int tcamNum) { tcam_num = tcamNum; }

int MemConfig::getSramNum() const { return sram_num; }

void MemConfig::setSramNum(int sramNum) { sram_num = sramNum; }

int MemConfig::getBitPerTcam() const { return bit_per_tcam; }

void MemConfig::setBitPerTcam(int bitPerTcam) { bit_per_tcam = bitPerTcam; }

int MemConfig::getBitPerSram() const { return bit_per_sram; }

void MemConfig::setBitPerSram(int bitPerSram) { bit_per_sram = bitPerSram; }
...
const std::unordered_map<int, LogicalTable*>&
MemConfig::getLogicalTableMap() const {
    return logical_table_map;
}

void MemConfig::setLogicalTableMap(
    const std::unordered_map<int, LogicalTable*>& logicalTableMap) {
...
void MemConfig::setFlowTableMap(
    const std::unordered_map<int, FlowTable*>& flowTableMap) {
    flow_table_map = flowTableMap;
}

const std::unordered_map<std::string, int>&
MemConfig::getFlowTableNameIdMap() const {
    return flow_table_name_id_map;
}

void MemConfig::setFlowTableNameIdMap(
    const std::unordered_map<std::string, int>& flowTableNameIdMap) {
...
void MemConfig::printFlowTableInfo() {
    for (auto& it : flow_table_map) {
        std::cout << "************** Flow Table " << it.first
                  << " **************" << std::endl;
        auto ft = it.second;
        std::cout << "flow table name: " << ft->getFlowTableName() << std::endl;
        std::cout << "logical table id: " << ft->getResidedLogicTableId()
                  << std::endl;
        std::cout << "logical table support length: "
                  << logical_table_map[ft->getResidedLogicTableId()]
                         ->getSupportedMaxLength()
                  << std::endl;
        std::cout << "logical table support num: "
                  << logical_table_map[ft->getResidedLogicTableId()]
                         ->getSupportedMaxNum()
                  << std::endl;
        std::cout << "logical table current capacity: "
                  << logical_table_map[ft->getResidedLogicTableId()]
                         ->getCurrentCapacity()
                  << std::endl;
        ...
        std::cout << "possible entry num: " << ft->getPossibleEntryNum()
                  << std::endl;
        std::cout << "current entry num: " << ft->getCurrentEntryNum()
                  << std::endl;
    }
}
...
void MemConfig::printSRAMInfo() {
    for (auto& sram : sram_map) {
        ...
        for (auto lt_id : sram.second->getLogicalTables()) {
            ...
            std::cout << "\t\tMatchType: "
                      << static_cast<bool>(lt->getMemType()) << std::endl;
...
void MemConfig::printTCAMInfo() {
    for (auto& tcam : tcam_map) {
        std::cout << "************** TCAM " << tcam.first << " **************"
                  << std::endl;
        std::cout << "Capacity: " << tcam.second->getCapacity() << std::endl;
        std::cout << "Free Capacity: " << tcam.second->getFreeCapacity()
                  << std::endl;

        auto lt_id = tcam.second->getLogicalTableId();
        std::cout << "\tLogical Table " << lt_id << ": " << std::endl;
        auto lt = logical_table_map[lt_id];
        std::cout << "\t\tMatchType: " << static_cast<bool>(lt->getMemType())
                  << std::endl;
        std::cout << "\t\tResidedTCAMId: " << lt->getResidedHardwareId()
                  << std::endl;
        std::cout << "\t\tSupportedMaxLength: " << lt->getSupportedMaxLength()
                  << std::endl;
        std::cout << "\t\tSupportedMaxNum: " << lt->getSupportedMaxNum()
                  << std::endl;
        std::cout << "\t\tCurrentCapacity: " << lt->getCurrentCapacity()
