...
void handleArgs(int argc, char* argv[], Parser* parser, MemConfig* mem_config,
                MatcherConfig* matcher, ExecuterConfig* executer_config,
    ...
    while ((opt = getopt_long(argc, argv, optstring, long_options,
        ...
        case 5: {
            matcher->initFromJson(config_file_path + optarg, parser, mem_config,
                                  processor);
            break;
        }
        case 6: {
            executer_config->initExecuterFromJson(config_file_path + optarg,
                                                  matcher, processor);
...
void handleRuntimeConfig(char* input, Parser* parser, MemConfig* mem_config,
                         MatcherConfig* matcher,
                         ExecuterConfig* executer_config, Processor* processor,
                         Metadata* metadata) {
    std::vector<std::string> tokens;
    split(input, tokens, " ");
    int a = std::find(instructions.begin(), instructions.end(), tokens[0]) -
            instructions.begin();
    switch (a) {
    case 1: {
...
int main(int argc, char** argv) {
    ...
    handleArgs(argc, argv, parser, mem_config, matcher, executer_config,
               processor, metadata);

    char input[255] = "";
    std::cout << "main> ";
    while (std::cin.getline(input, 254)) {
        handleRuntimeConfig(input, parser, mem_config, matcher, executer_config,
                            processor, metadata);
        std::cout << "main> ";
    }
    return 0;
