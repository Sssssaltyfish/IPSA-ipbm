 ...
 * Value. \internal ValueInternalMap is a tradional bucket based hash-table,
 * with a linked list in each bucket to handle collision. There is an addional
 * twist in that each node of the collision linked list is a page containing a
 * fixed amount of value. This provides a better compromise between memory usage
 * and speed.
 *
 * Each bucket is made up of a chained list of ValueInternalLink. The last
 * link of a given bucket can be found in the 'previous_' field of the following
 * bucket. The last link of the last bucket is stored in tailLink_ as it has no
 * following bucket. Only the last link of a bucket may contains 'available'
 * item. The last link always contains at least one element unless is it the
 * bucket one very first link.
 */
 ...
 * items. Instead of using a linked-list, a array of pointer is used for fast
 * item look-up. Look-up for an element is as follow:
 * - compute page index: pageIndex = itemIndex / itemsPerPage
 * - look-up item in page: pages_[pageIndex][itemIndex % itemsPerPage]
 *
 * Insertion is amortized constant time (only the array containing the index of
 * pointers need to be reallocated when items are appended).
 */
...
&indexCount, ValueInternalArray::PageIndex minNewIndexCount )
{
...
public:
    ...
    newArrayCopy(const ValueInternalArray& other) = 0;
