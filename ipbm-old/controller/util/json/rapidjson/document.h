...
// number of array elements that rapidjson::Value allocates memory for by
// default
...
public:
    ...
    /*! \param rhs Source of the assignment. Its name and value will become a
     * null value after assignment.
     */
    GenericMember& operator=(GenericMember& rhs) RAPIDJSON_NOEXCEPT {
...
public:
    ...
    */
    ...
    bool operator==(
        const GenericMemberIterator<Const_, Encoding, Allocator>& that) const {
...
public:
    ...
    */
    ...
    /*! Creates a copy of a Value by using the given Allocator
       ...
       Commonly use GenericDocument::GetAllocator(). \param copyConstStrings
       Force copying of constant strings (e.g. referencing an in-situ buffer)
        \see CopyFrom()
    */
    ...
    */
    ...
    /*!
       ...
       std::swap; swap(a.value, b.value);
        ...
        \see Swap()
     */
    friend inline void swap(GenericValue& a,
                            GenericValue& b) RAPIDJSON_NOEXCEPT {
        a.Swap(b);
    }
    ...
    /*!
       ...
       Object's member number and linear for the rest (number of all values in
       the subtree and total lengths of all strings).
    */
    template <typename SourceAllocator>
        ...
        case kObjectType:
            ...
            for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m) {
                ...
                if (RAPIDJSON_UNLIKELY(!handler.Key(
                        m->name.GetString(), m->name.GetStringLength(),
...
private:
    ...
    enum {
        ...
        kNullFlag = kNullType,
        // These casts are added to suppress the warning on MSVC about bitwise
        // operations between enums of different types.
        ...
        kNumberIntFlag = static_cast<int>(kNumberType) |
                         static_cast<int>(kNumberFlag | kIntFlag | kInt64Flag),
        kNumberUintFlag = static_cast<int>(kNumberType) |
                          static_cast<int>(kNumberFlag | kUintFlag |
                                           kUint64Flag | kInt64Flag),
        kNumberInt64Flag = static_cast<int>(kNumberType) |
                           static_cast<int>(kNumberFlag | kInt64Flag),
        kNumberUint64Flag = static_cast<int>(kNumberType) |
                            static_cast<int>(kNumberFlag | kUint64Flag),
        kNumberDoubleFlag = static_cast<int>(kNumberType) |
                            static_cast<int>(kNumberFlag | kDoubleFlag),
        kNumberAnyFlag =
            static_cast<int>(kNumberType) |
        ...
        kCopyStringFlag = static_cast<int>(kStringType) |
                          static_cast<int>(kStringFlag | kCopyFlag),
        kShortStringFlag =
            static_cast<int>(kStringType) |
    ...
    // destructor.
    void SetArrayRaw(GenericValue* values, SizeType count,
                     Allocator& allocator) {
        data_.f.flags = kArrayFlag;
        if (count) {
            ...
            std::memcpy(static_cast<void*>(e), values,
    ...
    void SetStringRaw(StringRefType s, Allocator& allocator) {
        ...
        if (ShortString::Usable(s.length)) {
            ...
            data_.s.length = s.length;
            str =
                static_cast<Ch*>(allocator.Malloc((s.length + 1) * sizeof(Ch)));
