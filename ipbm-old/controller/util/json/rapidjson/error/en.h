...
*/
inline const RAPIDJSON_ERROR_CHARTYPE*
GetValidateError_En(ValidateErrorCode validateErrorCode) {
    ...
    case kValidateErrorMultipleOf:
        return RAPIDJSON_ERROR_STRING("Number '%actual' is not a multiple of "
                                      "the 'multipleOf' value '%expected'.");
    case kValidateErrorMaximum:
        return RAPIDJSON_ERROR_STRING("Number '%actual' is greater than the "
                                      "'maximum' value '%expected'.");
    case kValidateErrorExclusiveMaximum:
        return RAPIDJSON_ERROR_STRING(
            "Number '%actual' is greater than or equal to the "
    ...
    case kValidateErrorExclusiveMinimum:
        return RAPIDJSON_ERROR_STRING(
            "Number '%actual' is less than or equal to the 'exclusiveMinimum' "
            "value '%expected'.");

    case kValidateErrorMaxLength:
        return RAPIDJSON_ERROR_STRING("String '%actual' is longer than the "
                                      "'maxLength' value '%expected'.");
    case kValidateErrorMinLength:
        return RAPIDJSON_ERROR_STRING("String '%actual' is shorter than the "
                                      "'minLength' value '%expected'.");
    case kValidateErrorPattern:
        return RAPIDJSON_ERROR_STRING("String '%actual' does not match the "
                                      "'pattern' regular expression.");

    case kValidateErrorMaxItems:
        return RAPIDJSON_ERROR_STRING("Array of length '%actual' is longer "
                                      "than the 'maxItems' value '%expected'.");
    case kValidateErrorMinItems:
        return RAPIDJSON_ERROR_STRING("Array of length '%actual' is shorter "
                                      "than the 'minItems' value '%expected'.");
    case kValidateErrorUniqueItems:
        return RAPIDJSON_ERROR_STRING(
            "Array has duplicate items at indices '%duplicates' but "
            "'uniqueItems' is true.");
    case kValidateErrorAdditionalItems:
        return RAPIDJSON_ERROR_STRING(
            "Array has an additional item at index '%disallowed' that is not "
            "allowed by the schema.");

    case kValidateErrorMaxProperties:
        return RAPIDJSON_ERROR_STRING(
            "Object has '%actual' members which is more than 'maxProperties' "
            "value '%expected'.");
    case kValidateErrorMinProperties:
        return RAPIDJSON_ERROR_STRING(
            "Object has '%actual' members which is less than 'minProperties' "
            "value '%expected'.");
    case kValidateErrorRequired:
        return RAPIDJSON_ERROR_STRING("Object is missing the following members "
                                      "required by the schema: '%missing'.");
    case kValidateErrorAdditionalProperties:
        return RAPIDJSON_ERROR_STRING(
            "Object has an additional member '%disallowed' that is not allowed "
            "by the schema.");
    case kValidateErrorPatternProperties:
        return RAPIDJSON_ERROR_STRING("Object has 'patternProperties' that are "
                                      "not allowed by the schema.");
    case kValidateErrorDependencies:
        return RAPIDJSON_ERROR_STRING(
            "Object has missing property or schema dependencies, refer to "
            "following errors.");

    case kValidateErrorEnum:
        return RAPIDJSON_ERROR_STRING("Property has a value that is not one of "
                                      "its allowed enumerated values.");
    case kValidateErrorType:
        return RAPIDJSON_ERROR_STRING(
            "Property has a type '%actual' that is not in the following list: "
            "'%expected'.");

    case kValidateErrorOneOf:
        return RAPIDJSON_ERROR_STRING(
            "Property did not match any of the sub-schemas specified by "
            "'oneOf', refer to following errors.");
    case kValidateErrorOneOfMatch:
        return RAPIDJSON_ERROR_STRING("Property matched more than one of the "
                                      "sub-schemas specified by 'oneOf'.");
    case kValidateErrorAllOf:
        return RAPIDJSON_ERROR_STRING(
            "Property did not match all of the sub-schemas specified by "
