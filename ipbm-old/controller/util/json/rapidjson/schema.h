...
#define RAPIDJSON_INVALID_KEYWORD_RETURN(code)                                 \
    ...
    context.invalidKeyword =                                                   \
        SchemaType::GetValidateErrorKeyword(code).GetString();                 \
...
enum ValidateFlag {
    ...
    kValidateContinueOnErrorFlag =
        1, //!< Don't stop after first validation error.
    kValidateDefaultFlags =
        RAPIDJSON_VALIDATE_DEFAULT_FLAGS //!< Default validate flags. Can be
...
public:
    ...
    virtual ISchemaValidator*
    CreateSchemaValidator(const SchemaType&,
...
public:
    ...
    virtual void TooShort(const Ch* str, SizeType length,
                          SizeType expected) = 0;
    ...
    virtual void NotOneOf(ISchemaValidator** subvalidators, SizeType count,
                          bool matched) = 0;
...
template <typename SchemaDocumentType> struct SchemaValidationContext {
    ...
    SchemaValidationContext(SchemaValidatorFactoryType& f, ErrorHandlerType& eh,
        ...
        : factory(f), error_handler(eh), schema(s), valueSchema(),
          invalidKeyword(), invalidCode(), hasher(), arrayElementHashCodes(),
          validators(), validatorCount(), patternPropertiesValidators(),
...
public:
    ...
    Schema(SchemaDocumentType* schemaDocument, const PointerType& p,
        ...
        : allocator_(allocator), uri_(schemaDocument->GetURI(), *allocator),
          ...
          additionalItems_(true), uniqueItems_(false), pattern_(),
          minLength_(0), maxLength_(~SizeType(0)), exclusiveMinimum_(false),
            ...
            if (v->IsArray() && v->Size() > 0) {
                ...
                for (ConstValueIterator itr = v->Begin(); itr != v->End();
                    ...
                    typedef Hasher<EncodingType, MemoryPoolAllocator<>>
                        EnumHasherType;
                    char buffer[256u + 24];
                    MemoryPoolAllocator<> hasherAllocator(buffer,
                                                          sizeof(buffer));
        ...
        if (context.inArray) {
            ...
            if (itemsList_)
                ...
                if (context.arrayElementIndex < itemsTupleCount_)
                    ...
                    context.error_handler.DisallowedItem(
                        context.arrayElementIndex);
                    // Must set valueSchema for when
                    // kValidateContinueOnErrorFlag is set, else reports
                    // spurious type error
                    context.valueSchema = typeless_;
                    // Must bump arrayElementIndex for when
                    // kValidateContinueOnErrorFlag is set
                    context.arrayElementIndex++;
                    RAPIDJSON_INVALID_KEYWORD_RETURN(
    ...
    RAPIDJSON_FORCEINLINE bool EndValue(Context& context) const {
        ...
        if (allOf_.schemas)
            for (SizeType i = allOf_.begin; i < allOf_.begin + allOf_.count;
                ...
                if (!context.validators[i]->IsValid()) {
                    context.error_handler.NotAllOf(
        ...
        if (oneOf_.schemas) {
            ...
            if (!oneValid) {
                context.error_handler.NotOneOf(
                    &context.validators[oneOf_.begin], oneOf_.count, false);
    ...
    bool String(Context& context, const Ch* str, SizeType length, bool) const {
        ...
        if (minLength_ != 0 || maxLength_ != SizeType(~0)) {
            SizeType count;
            if (internal::CountStringCodePoint<EncodingType>(str, length,
                                                             &count)) {
                if (count < minLength_) {
    ...
    bool StartObject(Context& context) const {
        ...
        if (hasDependencies_ || hasRequired_) {
            ...
            std::memset(context.propertyExist, 0,
                        sizeof(bool) * propertyCount_);
        }
    ...
    bool Key(Context& context, const Ch* str, SizeType len, bool) const {
        ...
        if (context.patternPropertiesSchemaCount ==
            ...
            // Must set valueSchema for when kValidateContinueOnErrorFlag is
            // set, else reports spurious type error
            context.valueSchema = typeless_;
    ...
    bool EndObject(Context& context, SizeType memberCount) const {
            ...
            for (SizeType index = 0; index < propertyCount_; index++)
                if (properties_[index].required &&
                    ...
                    if (properties_[index].schema->defaultValueLength_ == 0)
                        context.error_handler.AddMissingProperty(
        ...
        if (memberCount > maxProperties_) {
            context.error_handler.TooManyProperties(memberCount,
                                                    maxProperties_);
            ...
            for (SizeType sourceIndex = 0; sourceIndex < propertyCount_;
                ...
                if (context.propertyExist[sourceIndex]) {
                    if (source.dependencies) {
                        ...
                        context.error_handler.EndMissingDependentProperties(
                            source.name);
                    } else if (source.dependenciesSchema) {
                        ISchemaValidator* dependenciesValidator =
                            context
                                .validators[source.dependenciesValidatorIndex];
        ...
        if (!minimum_.IsNull()) {
            if (minimum_.IsInt64()) {
                if (exclusiveMinimum_ ? i <= minimum_.GetInt64()
                                      : i < minimum_.GetInt64()) {
                    context.error_handler.BelowMinimum(i, minimum_,
                                                       exclusiveMinimum_);
                    RAPIDJSON_INVALID_KEYWORD_RETURN(
                        exclusiveMinimum_ ? kValidateErrorExclusiveMinimum
                ...
                RAPIDJSON_INVALID_KEYWORD_RETURN(
                    exclusiveMinimum_
                        ...
                        : kValidateErrorMinimum); // i <= max(int64_t) <
                                                  // minimum.GetUint64()
        ...
        if (!maximum_.IsNull()) {
            if (maximum_.IsInt64()) {
                if (exclusiveMaximum_ ? i >= maximum_.GetInt64()
                                      : i > maximum_.GetInt64()) {
                    context.error_handler.AboveMaximum(i, maximum_,
                                                       exclusiveMaximum_);
                    RAPIDJSON_INVALID_KEYWORD_RETURN(
                        exclusiveMaximum_ ? kValidateErrorExclusiveMaximum
...
public:
    ...
    /*!
        ...
        \param uri The base URI of this schema document for purposes of
       violation reporting. \param uriLength Length of \c name, in code points.
    ...
    */
    explicit GenericSchemaDocument(
        ...
        : remoteProvider_(remoteProvider), allocator_(allocator),
          ownAllocator_(), root_(), typeless_(),
            ...
            if (const SchemaType* s = GetSchema(refEntry->target)) {
                ...
                if (!GetSchema(refEntry->source)) {
                    new (schemaMap_.template Push<SchemaEntry>()) SchemaEntry(
                        refEntry->source, const_cast<SchemaType*>(s), false,
                        allocator_);
                }
                            ...
                            if (pointer.IsValid()) {
                                if (const SchemaType* sc =
                                    ...
                                    if (schema)
                                        *schema = sc;
                                    new (
                                        schemaMap_.template Push<SchemaEntry>())
                                        SchemaEntry(source,
    ...
    */
    GenericSchemaValidator(
        ...
        : schemaDocument_(&schemaDocument), root_(schemaDocument.GetRoot()),
          ...
          error_(kObjectType), currentError_(), missingDependents_(),
          valid_(true), flags_(kValidateDefaultFlags)
#if RAPIDJSON_SCHEMA_VERBOSE
          ,
          depth_(0)
#endif
    ...
    */
    GenericSchemaValidator(
        ...
        : schemaDocument_(&schemaDocument), root_(schemaDocument.GetRoot()),
          ...
          missingDependents_(), valid_(true), flags_(kValidateDefaultFlags)
#if RAPIDJSON_SCHEMA_VERBOSE
          ,
          depth_(0)
#endif
    ...
    PointerType GetInvalidSchemaPointer() const {
        return schemaStack_.Empty() ? PointerType()
    ...
    //  If reporting all errors, the stack will be empty, so return
    //  kValidateErrors.
    ...
    void NotMultipleOf(int64_t actual, const SValue& expected) {
        AddNumberError(kValidateErrorMultipleOf, ValueType(actual).Move(),
                       expected);
    }
    ...
    void AboveMaximum(int64_t actual, const SValue& expected, bool exclusive) {
        AddNumberError(exclusive ? kValidateErrorExclusiveMaximum
                                 : kValidateErrorMaximum,
    ...
    void BelowMinimum(int64_t actual, const SValue& expected, bool exclusive) {
        AddNumberError(exclusive ? kValidateErrorExclusiveMinimum
                                 : kValidateErrorMinimum,
                       ValueType(actual).Move(), expected,
    ...
    void TooFewItems(SizeType actualCount, SizeType expectedCount) {
        AddNumberError(kValidateErrorMinItems, ValueType(actualCount).Move(),
                       SValue(expectedCount).Move());
    }
    void TooManyItems(SizeType actualCount, SizeType expectedCount) {
        AddNumberError(kValidateErrorMaxItems, ValueType(actualCount).Move(),
                       SValue(expectedCount).Move());
    }
    ...
    void PropertyViolations(ISchemaValidator** subvalidators, SizeType count) {
        for (SizeType i = 0; i < count; ++i)
            MergeError(static_cast<GenericSchemaValidator*>(subvalidators[i])
                           ->GetError());
    }
    ...
    void EndMissingDependentProperties(const SValue& sourceName) {
        if (!missingDependents_.Empty()) {
            ...
            error.AddMember(GetMissingString(), missingDependents_.Move(),
                            GetStateAllocator());
            ...
            AddErrorSchemaLocation(
                error, schemaRef.Append(
                           sourceName.GetString(), sourceName.GetStringLength(),
                           &GetInvalidSchemaPointer().GetAllocator()));
            ValueType wrapper(kObjectType);
            wrapper.AddMember(
                ValueType(SchemaType::GetValidateErrorKeyword(code),
                          GetStateAllocator())
                    .Move(),
                error, GetStateAllocator());
            currentError_.AddMember(
                ValueType(sourceName, GetStateAllocator()).Move(), wrapper,
                GetStateAllocator());
        }
    }
    ...
    void EndDisallowedType(const typename SchemaType::ValueType& actualType) {
        ValueType error(kObjectType);
        error.AddMember(GetExpectedString(), currentError_,
                        GetStateAllocator());
    ...
    void NotOneOf(ISchemaValidator** subvalidators, SizeType count,
                  bool matched = false) {
        AddErrorArray(matched ? kValidateErrorOneOfMatch : kValidateErrorOneOf,
                      subvalidators, count);
    }
    void Disallowed() {
        currentError_.SetObject();
...
#define RAPIDJSON_STRING_(name, ...)                                           \
    ...
    RAPIDJSON_STRING_(ErrorMessage, 'e', 'r', 'r', 'o', 'r', 'M', 'e', 's', 's',
                      'a', 'g', 'e')
...
#define RAPIDJSON_SCHEMA_HANDLE_END_(method, arg2)                             \
    valid_ = (EndValue() || GetContinueOnErrors()) &&                          \
             (!outputHandler_ || outputHandler_->method arg2);                 \
    return valid_;
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
    ...
    CreateSchemaValidator(const SchemaType& root,
        ...
        ISchemaValidator* sv =
            new (GetStateAllocator().Malloc(sizeof(GenericSchemaValidator)))
...
private:
    ...
    bool BeginValue() {
        if (schemaStack_.Empty())
            ...
            if (CurrentContext().inArray)
                internal::TokenHelper<internal::Stack<StateAllocator>, Ch>::
                    AppendIndexToken(documentStack_,
            ...
            if (count > 0) {
                ...
                for (SizeType i = 0; i < count; i++)
                    va[validatorCount++] = CreateSchemaValidator(
                        *sa[i], true); // Inherit continueOnError
            }
        ...
        if (!schemaStack_.Empty()) {
            ...
            if (hasher && context.valueUniqueness) {
                ...
                for (typename HashCodeArray::ConstValueIterator itr =
                     ...
                     itr != a->End(); ++itr)
                    if (itr->GetUint64() == h) {
                        DuplicateItems(static_cast<SizeType>(itr - a->Begin()),
                                       a->Size());
    ...
    RAPIDJSON_FORCEINLINE void PushSchema(const SchemaType& schema) {
        new (schemaStack_.template Push<Context>())
    ...
    void AddErrorInstanceLocation(ValueType& result, bool parent) {
        ...
        ValueType instanceRef(sb.GetString(),
                              ...
                              GetStateAllocator());
        result.AddMember(GetInstanceRefString(), instanceRef,
                         GetStateAllocator());
    }

    void AddErrorSchemaLocation(ValueType& result,
                                PointerType schema = PointerType()) {
        GenericStringBuffer<EncodingType> sb;
        SizeType len = CurrentSchema().GetURI().GetStringLength();
        if (len)
            memcpy(sb.Push(len), CurrentSchema().GetURI().GetString(),
                   len * sizeof(Ch));
    ...
    void AddCurrentError(const ValidateErrorCode code, bool parent = false) {
        ...
        AddError(ValueType(SchemaType::GetValidateErrorKeyword(code),
                           GetStateAllocator(), false)
                     .Move(),
                 currentError_);
    }
    ...
    void
    AddNumberError(const ValidateErrorCode code, ValueType& actual,
    ...
    void AddErrorArray(const ValidateErrorCode code,
        ...
        for (SizeType i = 0; i < count; ++i)
            errors.PushBack(
                static_cast<GenericSchemaValidator*>(subvalidators[i])
                    ->GetError(),
                GetStateAllocator());
        currentError_.SetObject();
...
public:
    ...
    */
    SchemaValidatingReader(InputStream& is, const SchemaDocumentType& sd)
        : is_(is), sd_(sd), invalidSchemaKeyword_(),
          invalidSchemaCode_(kValidateErrorNone), error_(kObjectType),
          isValid_(true) {}
    ...
    template <typename Handler> bool operator()(Handler& handler) {
        GenericReader<SourceEncoding, typename SchemaDocumentType::EncodingType,
                      StackAllocator>
            reader;
        GenericSchemaValidator<SchemaDocumentType, Handler> validator(sd_,
                                                                      handler);
