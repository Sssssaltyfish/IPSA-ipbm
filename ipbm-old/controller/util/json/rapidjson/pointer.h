...
public:
    ...
    */
    GenericPointer Append(const std::basic_string<Ch>& name,
                          Allocator* allocator = 0) const {
        return Append(name.c_str(), static_cast<SizeType>(name.size()),
                      allocator);
    }
#endif
    ...
    */
    GenericPointer Append(const ValueType& token,
                          Allocator* allocator = 0) const {
        if (token.IsString())
            return Append(token.GetString(), token.GetStringLength(),
                          allocator);
        else {
            ...
            if (v->IsArray() && t->name[0] == '-' && t->length == 1) {
                ...
                if (v->IsArray()) {
                    ...
                    if (m == v->MemberEnd()) {
                        ...
                        m = v->MemberEnd();
                        v = &(--m)->value; // Assumes AddMember() appends at the
                                           // end
                        exist = false;
                    } else
                        v = &m->value;
...
private:
    ...
    */
    Ch* CopyFromRaw(const GenericPointer& rhs, size_t extraToken = 0,
        ...
        if (nameBufferSize > 0) {
            std::memcpy(nameBuffer_, rhs.nameBuffer_,
                        nameBufferSize * sizeof(Ch));
        }
            ...
            while (i < length && source[i] != '/') {
                ...
                if (uriFragment) {
                    ...
                    if (c == '%') {
                        ...
                        if (!Transcoder<UTF8<>, EncodingType>().Validate(is,
                                                                         os) ||
                            !is.IsValid()) {
                            parseErrorCode_ =
        ...
        for (Token* t = tokens_; t != tokens_ + tokenCount_; ++t) {
            ...
            for (size_t j = 0; j < t->length; j++) {
                ...
                if (c == '~') {
                    ...
                    if (!Transcoder<EncodingType, UTF8<>>().Validate(source,
                                                                     target))
                        return false;
                    j += source.Tell() - 1;
                } else
