...
enum class KeyType { IPv4, IPv6, MAC };
...
std::vector<std::string> split(std::string entry, std::string reg) {
    ...
    std::vector<std::string> res(
        std::sregex_token_iterator(entry.begin(), entry.end(), ws, -1),
...
uint8_t* convert_from_hex_to_binary(std::string) { return nullptr; }

uint8_t* convert_from_sint_to_binary(std::string) { return nullptr; }

uint8_t* convert_from_bin_to_binary(std::string) { return nullptr; }
