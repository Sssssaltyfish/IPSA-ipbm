...
struct ParserStage {
    ...
    void modify_trans_act(const uint8_t key[], const uint8_t value[]) {
        ...
        ps_trans_tbl[idx]->entry =
            uint32_t(key[1] << 24) + uint32_t(key[2] << 16) +
            uint32_t(key[3] << 8) + key[4]; // last 32 bits for match key.
        ps_trans_tbl[idx]->mask =
            (key[5] << 24) + (key[6] << 16) + (key[7] << 8) + key[8];
        ...
        std::cout << (int)ps_act_tbl[idx]->hdr_id << "~"
                  << ps_act_tbl[idx]->hdr_len << std::endl;
#endif

        for (int i = 0; i < value[4]; i++) {
            // hdr_id, internal_offset, field_length
            ps_act_tbl[idx]->trans_fds[i]->hdr_id = value[5 + i * 6]; // 8-bit
            ps_act_tbl[idx]->trans_fds[i]->internal_offset =
                (value[6 + i * 6] << 8) + value[7 + i * 6]; // 16-bit
            ps_act_tbl[idx]->trans_fds[i]->fd_len =
                (value[8 + i * 6] << 8) + value[9 + i * 6]; // 16-bit
            ps_act_tbl[idx]->trans_fds[i]->fd_type =
                ((value[10 + i * 6] == 0) ? FieldType::FIELD
                                          : FieldType::VALID);
        }
    }

    void modify_trans_entry(uint8_t state, uint32_t entry, uint32_t mask,
        ...
        for (int i = 0; i < trans_fd_num; i++) {
            // hdr_id, internal_offset, field_length
            ps_act_tbl[idx]->trans_fds[i]->hdr_id =
                trans_fds[i]->hdr_id; // 8-bit
            ps_act_tbl[idx]->trans_fds[i]->internal_offset =
                trans_fds[i]->internal_offset; // 16-bit
            ps_act_tbl[idx]->trans_fds[i]->fd_len =
                trans_fds[i]->fd_len; // 16-bit
            ps_act_tbl[idx]->trans_fds[i]->fd_type = trans_fds[i]->fd_type;
            //            LOG(INFO) << "      idx: " << (int)i;
            //            LOG(INFO) << "      header_id: " <<
            //            (int)trans_fds[i]->hdr_id; LOG(INFO) << "
            //            field_internal_offset: " <<
            //            (int)trans_fds[i]->internal_offset; LOG(INFO) << "
            //            field_length: " << (int)trans_fds[i]->fd_len;
            //            LOG(INFO) << "      field_type: " <<
            //            (int)trans_fds[i]->fd_type;
        }
    }

    static uint32_t get_field_int(
        PHV* phv,
        FieldInfo* trans_fd /*uint8_t hdr_id, LEN internal_offset, LEN len*/) {
    ...
    void parse(PHV* phv) {
        ...
        for (int i = 0; i < 256; i++) {
            if ((ps_trans_tbl[i]->state == phv->cur_state) &&
                (ps_trans_tbl[i]->entry ==
                 (phv->cur_trans_key & ps_trans_tbl[i]->mask))) {
                sram_index = i;
                //                phv->tcam_miss = false;
                break;
            }
        ...
        if (sram_index == -1) {
            if (phv->miss_act == 0) {
                ...
                LOG(INFO) << "          accept directly";
                return;
                //                sram_index = 255 - phv->cur_state; // point to
                //                default entry
            }
        }
        ...
        for (int i = 0; i < _trans_fd_num; i++) {
            ...
            _trans_key +=
                (_trans_key << sram_entry->trans_fds[i]->fd_len) + res;
        }
...
public:
    Parser() { parser_levels = new ParserStage[4]; }
    ...
    void modify_parser_entry_direct(int level, uint8_t state, uint32_t entry,
                                    ...
                                    uint8_t hdr_len, uint8_t next_state,
                                    uint8_t trans_fd_num,
                                    std::vector<FieldInfo*> trans_fds,
                                    uint8_t miss_act) {
        parser_levels[level].modify_trans_entry(
            state, entry, mask, hdr_id, hdr_len, next_state, trans_fd_num,
