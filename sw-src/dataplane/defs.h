...
enum class FieldType { FIELD, VALID, HIT, MISS };

struct FieldInfo { // 24
    uint8_t hdr_id;
    uint16_t internal_offset; // 2^8 = 256 bits = 32 bytes, enough for current
                              // version
    uint16_t fd_len;
    FieldType fd_type;
};
...
struct Sram {
    ...
    Sram() { memset(tbl, 0, sizeof(uint8_t) * (PHYSICAL_SRAM_CAPACITY / 8)); }
};
...
struct PHV {
    // the offset of metadata is (front_header_length), and the id can be set as
    // 15
...
static inline void print_packet(uint8_t* packet, int len) {
    ...
    std::cout << "Ether: " << mac2str(eth_hdr->ether_shost) << " -> "
              << mac2str(eth_hdr->ether_dhost) << std::endl;
    if (eth_hdr->ether_type == htons(ETHERTYPE_IP)) {
        ...
        std::cout << "IP: " << inet_ntoa({ip_hdr->saddr}) << " -> "
                  << inet_ntoa({ip_hdr->daddr}) << " ttl=" << (int)ip_hdr->ttl
                  << " cksum=" << ip_hdr->check << std::endl;
    } else {
        std::cout << "Unknown ether type " << (int)eth_hdr->ether_type
                  << std::endl;
    }
}
