...
// uint32_t get_op_val(Parameter para, PHV * phv, std::vector<ActionParam*>
// action_paras) {
//     uint32_t res = 0;
//     switch(para.type) {
...
//                 value += (uint32_t)phv->packet[i] << ((byte_end - 1 - i) *
//                 8);
//             }
//             res = value;
//             break;
//         }
//         case PT::PARAM : { // if it is value-, just extract it out; if it is
//         field-, should be larger than uint32_t
...
Data get_op_val_any(Parameter para, PHV* phv,
                    std::vector<ActionParam*> action_paras) {
    Data res{};
    LOG(INFO) << "      param type(constant, header, field, param): "
              << (int)para.type;
    switch (para.type) {
    case PT::CONSTANT: {
    ...
    case PT::FIELD: {
        ...
        LOG(INFO) << "          internal_offset: "
                  << para.val.field.internal_offset;
        ...
        fd_backup[byte_len - 1] = fd_backup[byte_len - 1] >> right_shift
                                                                 << right_shift;

        int idx = fd_len_byte - 1;
        for (int i = byte_len - 1; i > 0; i--) {
            res.val[idx] = (fd_backup[i] >> right_shift) +
                           (fd_backup[i - 1] << (8 - right_shift));
            idx--;
    ...
    case PT::PARAM: { // if it is value-, just extract it out; if it is field-,
                      // should be larger than uint32_t
        ActionParam* ap = action_paras[para.val.action_para_id];
...
public:
    ...
    void set_lvalue(FieldInfo* fdInfo) { lvalue = fdInfo; }

    ExpTreeNode* get_root() { return this->root; }

    void set_root(ExpTreeNode* r) { root = r; }
    ...
    void run(PHV* phv, const std::vector<ActionParam*>& action_paras) {
        ...
        p->val.field = {lvalue->hdr_id, lvalue->internal_offset, lvalue->fd_len,
                        lvalue->fd_type};
        LOG(INFO) << "      modify_field: ";
        LOG(INFO) << "          header_id: " << (int)p->val.field.hdr_id;
        LOG(INFO) << "          header_offset: "
                  << (int)phv->parsed_headers[p->val.field.hdr_id]->hdr_offset;
        LOG(INFO) << "          internal_offset: "
                  << (int)p->val.field.internal_offset;
        ...
        int begin_bit = phv->parsed_headers[p->val.field.hdr_id]->hdr_offset +
                        p->val.field.internal_offset;
    ...
    Data execute(PHV* phv, ExpTreeNode* etn,
                 const std::vector<ActionParam*>& action_paras, int of) {
        std::string off = "";
        for (int i = 0; i < of; i++)
            off += "\t";
        LOG(INFO) << off
                  << "node.type(constant, header, field, param, operator):  "
                  << (int)etn->opt.type;
        if (etn->opt.type == ParameterType::OPERATOR) {
            LOG(INFO) << off
                      << "   operator.op(+ -):  " << (int)etn->opt.val.op;
            // ony set_field and copy_field can exceed 32
            switch (etn->opt.val.op) {
            case OpCode::ADD: {
                uint32_t res =
                    get_value(execute(phv, etn->left, action_paras, of + 1)) +
            ...
            case OpCode::BIT_NEG: {
                ...
                //                    Data res = execute(phv, etn->left,
                //                    action_paras); auto *p = new Parameter();
                //                    p->type = ParameterType::FIELD;
                //                    p->val.field = {lvalue->hdr_id,
                //                    lvalue->internal_offset,
                //                                   lvalue->fd_len,
                //                                   lvalue->fd_type};
                //                    modify_field_with_data(*p, res, phv);
                //                    break;
    ...
    static void modify_field_with_data(Parameter para, Data data, PHV* phv) {
        ...
        //        int byte_end = end / 8 - (end_offset == 0 ? 1 : 0);//ceil(end
        //        * 1.0 / 8); //11 int byte_len = byte_end - byte_begin + 1;//6
        //
        //        int origin_len =  data.data_len / 8 + (data.data_len % 8 ? 1 :
        //        0);//6 int len = 0; if(end_offset == 0) {
        //            len = data.data_len / 8 + (data.data_len % 8 ? 1 : 0); //
        //            6
        //        } else {
        //            len = (data.data_len + (8 - end_offset)) / 8 +
        //            ((data.data_len + (8 - end_offset)) % 8 ? 1 : 0);
        //        }
        ////        int len = (data.data_len + (8 - end_offset)) / 8 +
        ///(data.data_len + (8 - end_offset)) % 8 ? 1 : 0;
        //
        //        auto * res = new uint8_t[len];
        //        memset(res, 0, len * sizeof(uint8_t));
        //
        //        int idx = len - 1; // 5    //00 00 00 00 01 50 -> 00 00 00 00
        //        01 01 // 00 00 00 00 00 00 // 01 01 for(int i = origin_len -
        //        1; i >= 0; i--) {
        //            res[idx] += (data.val[i] << (8 - (end_offset == 0 ? 8 :
        //            end_offset))); if(idx > 0){
        ...
        //                sd += (1 << j);
        //            }
        //            phv->packet[i] = (phv->packet[i] & sd) + res[idx--]; //
        //            here
        //        }
    }
...
//     }
//
//     static void modify_field_with_uint(Parameter para, uint32_t res, PHV *
//     phv) {
//         int internal_offset = para.val.field.internal_offset;
//         int fd_len = para.val.field.fd_len;
//         auto hi = phv->parsed_headers[para.val.field.hdr_id];
...
//
//         int byte_begin = begin / 8;
//         int byte_end = end / 8 - (end_offset == 0 ? 1 : 0);//ceil(end * 1.0 /
//         8); int byte_len = byte_end - byte_begin + 1;
//
...
//             len = data.data_len / 8 + (data.data_len % 8 ? 1 : 0);
//         } else {
//             len = (data.data_len + (8 - end_offset)) / 8 + ((data.data_len +
//             (8 - end_offset)) % 8 ? 1 : 0);
//         }
////        int len = (data.data_len + (8 - end_offset)) / 8 + (data.data_len +
///(8 - end_offset)) % 8 ? 1 : 0;
//
//        auto * res = new uint8_t[len];
//        memset(res, 0, len * sizeof(uint8_t));
...
//    }
//
//    void execute(PHV * phv, const std::vector<ActionParam*>& action_paras)
//    const {
//        switch(op) {
//            case OpCode::ADD: { // the first param must be field
//                // get second and third params
//                uint32_t res = get_value(get_op_val_any(params[1], phv,
...
struct Action {
    ...
    void set_action_paras(uint8_t* value) {
        ...
        for (int i = 0; i < para_num; i++) {
            ...
            value_new[0] =
                (uint8_t(value_new[0] << left_offset) >> left_offset);
            //            std::cout << "y: " << (int)value_new[0] << std::endl;
            value_new[new_len - 1] =
                right_offset == 0 ? value_new[new_len - 1]
                                  : (value_new[new_len - 1] >>
                                     (8 - right_offset) << (8 - right_offset));
            ...
            for (int j = byte_len - 1; j >= 0; j--) {
                if (right_offset == 0) {
                    ...
                    if (idx > 0) {
                        data->val[j] +=
                            uint8_t(value_new[idx - 1] << right_offset);
                    }
                }
    ...
    void execute(PHV* phv) {
        ...
        for (auto it : prims) {
            LOG(INFO) << "The type of lvalue of primitive: "
                      << (int)it->lvalue->fd_type << std::endl;
...
public:
    ...
    void execute(PHV* phv) {
        LOG(INFO) << "  Executor...";
        int32_t action_id =
            phv->next_action_id; // phv->match_value[1] +
                                 // ((uint32_t)phv->match_value[0] << 8);
