...
struct KeyInfo {
    int tail_offset; // e.g., 14-bit field, using 2 bytes, and last 2 bit is
                     // empty, so tail offset is 2
    uint8_t* field;
    int byte_len;
    LEN field_len;

    KeyInfo(int _tail_mod, uint8_t* fd, int _byte_length, LEN _field_len)
        : tail_offset(_tail_mod), field(fd), byte_len(_byte_length),
          field_len(_field_len) {}
};

enum class MatchType { EXACT, TERNARY, LPM };
...
public:
    ...
    MatcherThread(int _proc_id) {
        ...
        for (int i = 0; i < SRAM_NUM_PER_CLUSTER; i++) {
            sram[i] =
                glb.srams[proc_id / PROC_NUM_PER_CLUSTER * SRAM_IDX_BASE + i];
        }

        for (int i = 0; i < TCAM_NUM_PER_CLUSTER; i++) {
            tcam[i] =
                glb.tcams[proc_id / PROC_NUM_PER_CLUSTER * TCAM_IDX_BASE + i];
        }
    ...
    void
    set_action_proc_map(const std::unordered_map<int, int>& _action_proc_map) {
    ...
    void clear_old_config() {
        ...
        if (match_type == MatchType::EXACT) {
            // clear key sram
            //            std::cout << "sram_slice_key_width: " <<
            //            key_width.sram_slice_key_width << std::endl; std::cout
            //            << "depth.sram_slice_depth: " <<
            //            depth.sram_slice_depth << std::endl;
            for (int i = 0;
                 i < key_width.sram_slice_key_width * depth.sram_slice_depth;
                 i++) {
                memset(sram[key_config.sram_key_config[i]]->tbl, 0,
                       PHYSICAL_SRAM_CAPACITY / 8 * sizeof(uint8_t));
            }
            LOG(INFO) << "  key sram cleared";
            // clear value sram
            //            std::cout << "sram_slice_value_width: " <<
            //            sram_slice_value_width << std::endl; std::cout <<
            //            "depth.sram_slice_depth: " << depth.sram_slice_depth
            //            << std::endl;

            for (int i = 0; i < sram_slice_value_width * depth.sram_slice_depth;
                 i++) {
                memset(sram[sram_value_config[i]]->tbl, 0,
                       PHYSICAL_SRAM_CAPACITY / 8 * sizeof(uint8_t));
            }
            LOG(INFO) << "  value sram cleared";
            memset(key_config.sram_key_config, 0,
                   SRAM_NUM_PER_CLUSTER * sizeof(uint8_t));
            LOG(INFO) << "  key config cleared";
            memset(sram_value_config, 0,
                   SRAM_NUM_PER_CLUSTER * sizeof(uint8_t));
            ...
            // clear key tcam
            for (int i = 0;
                 i < key_width.tcam_slice_key_width * depth.tcam_slice_depth;
                 i++) {
                memset(tcam[key_config.tcam_key_config[i]]->tbl, 0,
                       PHYSICAL_TCAM_CAPACITY / 8 * sizeof(uint8_t));
            }
            // clear key mask
            for (int i = 0;
                 i < key_width.tcam_slice_key_width * depth.tcam_slice_depth;
                 i++) {
                memset(tcam[key_config.tcam_key_config[i]]->mask, 0,
                       PHYSICAL_TCAM_CAPACITY / 8 * sizeof(uint8_t));
            }
            // clear value sram
            for (int i = 0; i < sram_slice_value_width * depth.tcam_slice_depth;
                 i++) {
                memset(sram[sram_value_config[i]]->tbl, 0,
                       PHYSICAL_TCAM_CAPACITY / 8 * sizeof(uint8_t));
            }
            memset(key_config.tcam_key_config, 0,
                   TCAM_NUM_PER_CLUSTER * sizeof(uint8_t));
            memset(sram_value_config, 0,
                   TCAM_NUM_PER_CLUSTER * sizeof(uint8_t));
    ...
    void set_no_table(bool _no_table) { no_table = _no_table; }

    // should be called after init_match_type
    void set_mem_config(int _key_width, int _value_width, int _depth,
                        const uint8_t* _key_config,
                        const uint8_t* _value_config) {
        no_table = false;
        if (match_type == MatchType::EXACT) {
            ...
            for (int i = 0; i < SRAM_NUM_PER_CLUSTER; i++) {
                sram[i] =
                    glb.srams[proc_id / PROC_NUM_PER_CLUSTER * SRAM_IDX_BASE +
                              i];
            }
        } else {
            ...
            for (int i = 0; i < TCAM_NUM_PER_CLUSTER; i++) {
                tcam[i] =
                    glb.tcams[proc_id / PROC_NUM_PER_CLUSTER * TCAM_IDX_BASE +
                              i];
            }
    ...
    void set_miss_act_id(int _miss_act_id) { miss_act_id = _miss_act_id; }
    ...
    void set_field_info(std::vector<FieldInfo*> _fdInfos) {
        ...
        for (auto it : fdInfos) {
            ...
            LOG(INFO) << "          internal offset: "
                      << (int)it->internal_offset;
    ...
    void insert_sram_entry(uint8_t* key, uint8_t* value, int key_byte_len,
                           int value_byte_len) {
        ...
        for (int i = 0; i < key_width.sram_slice_key_width; i++) {
            sram_key_seq[i] =
                key_config
                    .sram_key_config[slice_idx + depth.sram_slice_depth * i];
        }
        for (int i = 0; i < sram_slice_value_width; i++) {
            sram_value_seq[i] =
                sram_value_config[slice_idx + depth.sram_slice_depth * i];
        }

        // key should be SRAM_WIDTH-aligned
        // should be right-aligned?
        for (int i = 0; i < key_width.sram_slice_key_width; i++) {
            int offset = (key_byte_len - i * SRAM_WIDTH / 8) >= SRAM_WIDTH / 8
                             ? SRAM_WIDTH / 8
                             : (key_byte_len - i * SRAM_WIDTH / 8);
            // assign the entry at the left of the SRAM
            memcpy(sram[sram_key_seq[i]]->tbl + (idx * SRAM_WIDTH / 8),
                   key + i * SRAM_WIDTH / 8, offset);
            // assign the entry at the right of the SRAM
            //            memcpy(sram[sram_key_seq[i]]->tbl + (idx * SRAM_WIDTH
            //            / 8) + (SRAM_WIDTH / 8 - offset), key + i * SRAM_WIDTH
            //            / 8 , offset);
        }

        for (int i = 0; i < sram_slice_value_width; i++) {
            int offset = (value_byte_len - i * SRAM_WIDTH / 8) >= SRAM_WIDTH / 8
                             ? SRAM_WIDTH / 8
                             : (value_byte_len - i * SRAM_WIDTH / 8);
            memcpy(sram[sram_value_seq[i]]->tbl + (idx * SRAM_WIDTH / 8),
                   value + i * SRAM_WIDTH / 8, offset);
            LOG(INFO) << "  sram value: ";
            for (int j = 0; j < sram_slice_value_width * SRAM_WIDTH / 8; j++) {
                LOG(INFO) << "      "
                          << (int)sram[sram_value_seq[i]]
                                 ->tbl[(idx * SRAM_WIDTH / 8) + j];
            }
            //            memcpy(sram[sram_value_seq[i]]->tbl + (idx *
            //            SRAM_WIDTH / 8) + (SRAM_WIDTH / 8 - offset), value + i
            //            * SRAM_WIDTH / 8 , offset);
        }
    }

    void insert_tcam_entry(uint8_t* key, uint8_t* mask, uint8_t* value,
        ...
        for (int i = 0; i < key_width.tcam_slice_key_width; i++) {
            tcam_key_seq[i] =
                key_config
                    .tcam_key_config[slice_idx + depth.tcam_slice_depth * i];
        }
        for (int i = 0; i < sram_slice_value_width; i++) {
            sram_value_seq[i] =
                sram_value_config[slice_idx + depth.tcam_slice_depth * i];
        }

        // key should be SRAM_WIDTH-aligned
        // should be right-aligned?
        for (int i = 0; i < key_width.tcam_slice_key_width; i++) {
            int offset = (key_byte_len - i * TCAM_WIDTH / 8) >= TCAM_WIDTH / 8
                             ? TCAM_WIDTH / 8
                             : (key_byte_len - i * TCAM_WIDTH / 8);
            // assign the entry at the left of the SRAM
            memcpy(tcam[tcam_key_seq[i]]->tbl + (idx * TCAM_WIDTH / 8),
                   key + i * TCAM_WIDTH / 8, offset);
            memcpy(tcam[tcam_key_seq[i]]->mask + (idx * TCAM_WIDTH / 8),
                   mask + i * TCAM_WIDTH / 8, offset);
        }
    ...
    // key should be aligned to SRAM width
    //    void insert_entry(uint8_t * key, uint8_t * value, int key_byte_len,
    //    int value_byte_len) {
    //        uint16_t key_sum = 0;
    //        for(int i = 0; i < key_byte_len; i++) {
    //            key_sum += key[i];
    ...
    //            sram_key_seq[i] = sram_key_config[slice_idx + sram_slice_depth
    //            * i];
    //        }
    //        for(int i = 0; i < sram_slice_value_width; i++) {
    //            sram_value_seq[i] = sram_value_config[slice_idx +
    //            sram_slice_depth * i];
    //        }
    //
    //        // key should be SRAM_WIDTH-aligned
    //        // should be right-aligned?
    //        for(int i = 0; i < sram_slice_key_width; i++) {
    //            int offset = (key_byte_len - i * SRAM_WIDTH / 8) >= SRAM_WIDTH
    //            / 8 ? SRAM_WIDTH / 8 : (key_byte_len - i * SRAM_WIDTH / 8);
    //            // assign the entry at the left of the SRAM
    //            memcpy(sram[sram_key_seq[i]]->tbl + (idx * SRAM_WIDTH / 8),
    //            key + i * SRAM_WIDTH / 8 , offset);
    //            // assign the entry at the right of the SRAM
    ////            memcpy(sram[sram_key_seq[i]]->tbl + (idx * SRAM_WIDTH / 8) +
    ///(SRAM_WIDTH / 8 - offset), key + i * SRAM_WIDTH / 8 , offset);
    //        }
    //
    //        for(int i = 0; i < sram_slice_value_width; i++) {
    //            int offset = (value_byte_len - i * SRAM_WIDTH / 8) >=
    //            SRAM_WIDTH / 8 ? SRAM_WIDTH / 8 : (value_byte_len - i *
    //            SRAM_WIDTH / 8); memcpy(sram[sram_value_seq[i]]->tbl + (idx *
    //            SRAM_WIDTH / 8), value + i * SRAM_WIDTH / 8 , offset);
    ////            memcpy(sram[sram_value_seq[i]]->tbl + (idx * SRAM_WIDTH / 8)
    ///+ (SRAM_WIDTH / 8 - offset), value + i * SRAM_WIDTH / 8 , offset);
    //        }
    //    }
    ...
    static KeyInfo* get_field(FieldInfo* fd, PHV* phv) {
        ...
        int left_empty = begin_idx % 8,
            right_empty = (end_idx + 1) % 8 == 0 ? 0 : (8 - (end_idx + 1) % 8);
        ...
        if (left_empty + right_empty >= 8) {
            field[byte_len - 1] =
                (phv->packet[begin_byte + byte_len - 1] << left_empty) +
                (phv->packet[begin_byte + byte_len] >> (8 - left_empty));
        } else {
            field[byte_len - 1] =
                (phv->packet[begin_byte + byte_len - 1] << left_empty);
        }
        ...
        for (int i = 0; i < byte_len - 1; i++) {
            field[i] = (phv->packet[begin_byte + i] << left_empty) +
                       (phv->packet[begin_byte + i + 1] >> (8 - left_empty));
        }
    ...
    static uint8_t* combine_to_single_key(const std::vector<KeyInfo*>& keyInfos,
                                          LEN fd_len_sum) {
        ...
        for (auto ki : keyInfos) {
            ...
            for (int i = 0; i < ki->byte_len; i++) {
                if (i < ki->byte_len - 1) {
                    match_key[cur_idx] =
                        match_key[cur_idx] + (fd[i] >> (8 - cur_bit_left));
        ...
        //        int right_shift = fd_len_sum % 8 == 0 ? 0 : (8 - fd_len_sum %
        //        8); for(int i = len - 1; i > 0; i--) {
        //            match_key[i] = (match_key[i] >> right_shift) +
        //            (match_key[i-1] << (8 - right_shift));
        //        }
    ...
    void get_match_res(const uint8_t* match_key, LEN key_len, uint8_t* out_key,
                       uint8_t* out_value,
                       int& hit) { //, uint8_t * out_key, uint8_t * out_value
        if (match_type == MatchType::EXACT) {
            ...
            for (int i = 0; i < key_width.sram_slice_key_width; i++) {
                sram_key_seq[i] =
                    key_config
                        .sram_key_config[base + depth.sram_slice_depth * i];
            }

            for (int i = 0; i < sram_slice_value_width; i++) {
                sram_value_seq[i] =
                    sram_value_config[base + depth.sram_slice_depth * i];
            }
            ...
            for (int i = 0; i < key_width.sram_slice_key_width; i++) {
                memcpy(out_key + i * SRAM_WIDTH / 8,
                       sram[sram_key_seq[i]]->tbl + (idx * SRAM_WIDTH / 8),
                       SRAM_WIDTH / 8);
            }

            LOG(INFO) << "         out key:";
            for (int i = 0; i < key_width.sram_slice_key_width * SRAM_WIDTH / 8;
                 i++) {
                LOG(INFO) << "              " << (int)out_key[i];
            }

            for (int i = 0; i < sram_slice_value_width; i++) {
                memcpy(out_value + i * SRAM_WIDTH / 8,
                       sram[sram_value_seq[i]]->tbl + (idx * SRAM_WIDTH / 8),
                       SRAM_WIDTH / 8);
            }
            ...
            for (int i = 0; i < key_width.tcam_slice_key_width; i++) {
                tcam_key_seq[i] =
                    key_config
                        .tcam_key_config[base + depth.tcam_slice_depth * i];
            }

            for (int i = 0; i < sram_slice_value_width; i++) {
                sram_value_seq[i] =
                    sram_value_config[base + depth.tcam_slice_depth * i];
            }

            auto tmp_key =
                new uint8_t[key_width.tcam_slice_key_width * TCAM_WIDTH / 8];
            auto tmp_mask =
                new uint8_t[key_width.tcam_slice_key_width * TCAM_WIDTH / 8];
            for (int i = 0; i < (1 << TCAM_DEPTH); i++) {
                for (int j = 0; j < key_width.tcam_slice_key_width; j++) {
                    memcpy(tmp_key + j * TCAM_WIDTH / 8,
                           tcam[tcam_key_seq[j]]->tbl + (i * TCAM_WIDTH / 8),
                           TCAM_WIDTH / 8);
                    memcpy(tmp_key + j * TCAM_WIDTH / 8,
                           tcam[tcam_key_seq[j]]->mask + (i * TCAM_WIDTH / 8),
                           TCAM_WIDTH / 8);
                }
                int flag = 0;
                ...
                if (flag == 0) {
                    for (int l = 0; l < key_width.tcam_slice_key_width; l++) {
                        memcpy(out_key + l * TCAM_WIDTH / 8,
                               tcam[tcam_key_seq[l]]->tbl +
                                   (i * TCAM_WIDTH / 8),
                               TCAM_WIDTH / 8);
                    }

                    for (int l = 0; l < sram_slice_value_width; l++) {
                        memcpy(out_value + l * SRAM_WIDTH / 8,
                               sram[sram_value_seq[l]]->tbl +
                                   (i * SRAM_WIDTH / 8),
                               SRAM_WIDTH / 8);
                    }
                    hit = 1;
    ...
    void
    generate_match_key_and_match(PHV* phv) { // std::vector<FieldInfo*> fdInfos,
        ...
        for (auto fi : fdInfos) {
            ...
            LOG(INFO) << "              field internal offset: "
                      << (int)fi->internal_offset;
            LOG(INFO) << "              field length: " << (int)fi->fd_len;
            LOG(INFO) << "              field type(field, valid, hit, miss): "
                      << (int)fi->fd_type;
        ...
        int key_byte_len =
            ((match_type == MatchType::EXACT)
                 ? key_width.sram_slice_key_width * SRAM_WIDTH / 8
        ...
        if (hit == 1) {
            ...
            phv->next_action_id =
                (uint32_t(out_value[1])) + ((uint32_t)out_value[0] << 8);
...
public:
    ...
    void insert_sram_entry(int matcher_id, uint8_t* key, uint8_t* value,
                           int key_byte_len, int value_byte_len) {
        mts[matcher_id]->insert_sram_entry(key, value, key_byte_len,
                                           value_byte_len);
    }

    void insert_tcam_entry(int matcher_id, uint8_t* key, uint8_t* mask,
                           uint8_t* value, int key_byte_len,
                           int value_byte_len) {
        mts[matcher_id]->insert_tcam_entry(key, mask, value, key_byte_len,
                                           value_byte_len);
    }
    ...
    void set_mem_config(int matcher_id, MatchType _match_type, int key_width,
                        int value_width, int depth, uint8_t* key_config,
                        uint8_t* value_config) {
        mts[matcher_id]->clear_old_config();
        mts[matcher_id]->init_match_type(_match_type);
        mts[matcher_id]->set_mem_config(key_width, value_width, depth,
