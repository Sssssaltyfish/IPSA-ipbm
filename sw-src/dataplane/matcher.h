...
// 3. get the sram or tcam indexes, calculate the sram or tcam blocks that need
// to be looked up.
...
public:
    ...
    std::vector<FieldInfo*>
        fdInfos; // the match fields info should be configured or bypassed

    int miss_act; // 0: none; 1: execute_default_action; 2: set_proc_id; 3.
                  // execute_default_action and set_proc_id
    int miss_proc_id;
    int hit_proc_id;
    ...
    void set_miss_act(int _miss_act) { miss_act = _miss_act; }

    void set_hit_proc_id(int _hit_proc_id) { hit_proc_id = _hit_proc_id; }

    void set_miss_proc_id(int _miss_proc_id) { miss_proc_id = _miss_proc_id; }
    ...
    KeyInfo* get_field(FieldInfo* fd, PHV* phv) {
        // All of the keys should be aligned forward, otherwise it is
        // resource-consuming and error-prone, though it is complex work
        LEN hdr_offset = phv->parsed_headers[fd->hdr_id]->hdr_offset;

        int begin = (hdr_offset + fd->internal_offset) / 8;
        int end = (hdr_offset + fd->internal_offset + fd->fd_len) /
                  8; // + ((fd->internal_offset + fd->fd_len) % 8 == 0 ? 0 : 1);
        ...
        if (tail_mod != 0) {
            field[end - begin] = field[end - begin] >> (8 - tail_mod)
                                                           << (8 - tail_mod);
        }
        ...
        if (head_mod != 0) {
            for (int i = 0; i < real_len - 1; i++) {
                field[i] =
                    (field[i] << head_mod) + (field[i + 1] >> (8 - head_mod));
            }
    ...
    uint8_t* combine_to_single_key(std::vector<KeyInfo*> keyInfos,
                                   LEN fd_len_sum) {
    ...
    void get_match_res(
        uint8_t* match_key, LEN key_len, uint8_t* out_key,
