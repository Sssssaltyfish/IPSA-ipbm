...
uint8_t* combine_to_single_key(const std::vector<KeyInfo*>& keyInfos,
                               LEN fd_len_sum) {
    ...
    //            match_key[i] = (match_key[i] >> right_shift) + (match_key[i-1]
    //            << (8 - right_shift));
    //        }
...
uint8_t* extract_action_para(json j, int& value_width) {
    ...
    for (auto& para : j) {
        ...
        for (int i = 0; i < width_byte - 1; i++) {
            para_byte[i] = (para_byte[i] << empty_len) +
                           (para_byte[i + 1] >> (8 - empty_len));
            LOG(INFO) << "      [" << i << "]: " << (int)para_byte[i];
        }
        para_byte[width_byte - 1] = para_byte[width_byte - 1] << empty_len;
        LOG(INFO) << "      [" << width_byte - 1
                  << "]: " << (int)para_byte[width_byte - 1];
...
uint8_t* extract_exact(json j, int& key_width) {
    ...
    for (auto& key : j) {
        ...
        int empty_len = width_byte * 8 - width;
        for (int i = 0; i < width_byte - 1; i++) {
            key_byte[i] = (key_byte[i] << empty_len) +
                          (key_byte[i + 1] >> (8 - empty_len));
        }
...
std::pair<uint8_t*, uint8_t*> extract_ternary(json j, int& key_width) {
    ...
    for (auto& key : j) {
        ...
        for (int i = 0; i < width_byte - 1; i++) {
            key_byte[i] = (key_byte[i] << empty_len) +
                          (key_byte[i + 1] >> (8 - empty_len));
            mask_byte[i] = (mask_byte[i] << empty_len) +
                           (mask_byte[i + 1] >> (8 - empty_len));
        }
        key_byte[width_byte - 1] = key_byte[width_byte - 1] << empty_len;
...
void extract(const std::string& json_path, api::CfgClient& cfg) {
    ...
    int rc;
    if (match_type == MatchType::EXACT) {
        rc = cfg.insert_sram_entry(proc_id, matcher_id, key, value, key_width,
                                   value_width);
    } else {
        rc = cfg.insert_tcam_entry(proc_id, matcher_id, key, mask, value,
