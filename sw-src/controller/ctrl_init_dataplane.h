...
void extract_parser(json j, int proc_id, api::CfgClient& cfg) {
    ...
    for (auto& level_json : j.at("levels")) {
        for (auto& parser_entry_json : level_json) {
            ...
            int current_state =
                parser_tcam_entry.at("current_state").get<int>();
            ...
            int header_length =
                parser_sram_entry.at("header_length").get<int>();
            int next_state = parser_sram_entry.at("next_state").get<int>();
            int transition_field_num =
                parser_sram_entry.at("transition_field_num").get<int>();
            ...
            for (auto& tf : parser_sram_entry.at("transition_fields")) {
                FieldInfo fd = {
                    ...
                    static_cast<uint16_t>(
                        tf.at("field_internal_offset").get<int>()),
...
#ifndef NO_CFG
            int rc =
                cfg.mod_parser_entry(proc_id, level_id, current_state, entry,
                                     mask, header_id, header_length, next_state,
                                     transition_field_num, fd_infos, miss_act);
...
RelationCode get_relation_code(const std::string& op) {
    std::unordered_map<std::string, RelationCode> op_map = {
        {"==", RelationCode::EQ}, {"!=", RelationCode::NEQ},
        {">", RelationCode::GT},  {">=", RelationCode::GTE},
        {"<", RelationCode::LT},  {"<=", RelationCode::LTE}};
    return op_map[op];
}
...
void set_gate_param(GateParam& gp, json j) {
    ...
    case ParameterType::CONSTANT: {
        gp.constant_data = {32, get_uints(j.at("value").get<int>(), 32)};
        break;
    }
...
GateEntryType get_gate_entry_type(const std::string& get) {
    std::unordered_map<std::string, GateEntryType> get_map = {
        {"TABLE", GateEntryType::TABLE}, {"STAGE", GateEntryType::STAGE}};
    return get_map[get];
}
...
#ifndef NO_CFG
    int rc = cfg.set_default_gate_entry(
        proc_id,
...
void extract_matcher(json j, int proc_id, api::CfgClient& cfg) {
    ...
    for (auto& matcher_thread_json : j) {
        ...
        MatchType mt = get_match_type(
            matcher_thread_json.at("match_type").get<std::string>());
...
#ifndef NO_CFG
        int rc = cfg.set_mem_config(proc_id, matcher_id, key_width, value_width,
                                    key_config, value_config, mt, depth,
                                    miss_act_id);
        ...
        for (auto& fd_info : matcher_thread_json.at("field_infos")) {
            auto fd = new FieldInfo{
                ...
                static_cast<uint16_t>(
                    fd_info.at("field_internal_offset").get<int>()),
...
OpCode get_op_code(const std::string& op) {
    std::unordered_map<std::string, OpCode> op_map = {
        ...
        {"&", OpCode::BIT_AND},      {"|", OpCode::BIT_OR},
        {"~", OpCode::BIT_NEG},      {"^", OpCode::BIT_XOR},
        {">>", OpCode::SHIFT_RIGHT}, {"<<", OpCode::SHIFT_LEFT}};
    return op_map[op];
}
...
void build_exp_tree(ExpTreeNode* node, json j_node) {
    ...
    case ParameterType::CONSTANT: {
        node->opt.type = node_type;
        node->opt.val.constant_data = {
            32, get_uints(j_node.at("value").get<int>(), 32)};
        break;
    }
    default:
        break;
    ...
    for (auto& action : j) {
        ...
        for (auto& primitive : action.at("primitives")) {
            ...
            prim->lvalue = new FieldInfo{
                ...
                static_cast<uint16_t>(
                    lvalue.at("field_internal_offset").get<int>()),
...
#ifndef NO_CFG
        int rc = cfg.insert_action(proc_id, action_id, parameter_num, prims,
                                   parameter_lengths);
