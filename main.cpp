...
PHV* init_phv(uint8_t packet[], uint32_t recv_len, int if_index,
              Pipeline* pipe) {
    PHV* phv = new PHV();
    ...
    for (auto meta : pipe->get_metadata_header()) {
        ...
        phv->parsed_headers[meta->hdr_id]->hdr_offset =
            meta->hdr_offset * 8 + FRONT_HEADER_LEN * 8;
        LOG(INFO) << "MataData: ";
        LOG(INFO) << "  header_id: "
                  << phv->parsed_headers[meta->hdr_id]->hdr_id;
        LOG(INFO) << "  header_len: "
                  << phv->parsed_headers[meta->hdr_id]->hdr_len;
            ...
            if (out_port) {
                ...
                for (int if_idx = 0; if_idx < glb.num_if; if_idx++) {
                    if ((out_port >> if_idx) & 1) {
                        ...
                        static const uint8_t ZERO_MAC[] =
                            "\x00\x00\x00\x00\x00\x00";
                        if (memcmp(&glb.if_macs[if_idx], ZERO_MAC,
                                   sizeof(ether_addr)) != 0) {
#ifdef PIPE
                            memcpy(packet, phv->packet,
                                   recv_len * sizeof(uint8_t));
                            print_packet(packet, recv_len);
#endif
                            auto eth_hdr = (ether_header*)packet;
                            memcpy(eth_hdr->ether_shost, &glb.if_macs[if_idx],
                                   sizeof(ether_addr));
//                        print_packet(packet, recv_len);
#ifndef PIPE
                            if (if_idx == 0) {
                                memcpy(eth_hdr->ether_dhost, MAC1,
                                       sizeof(ether_addr));
                            } else if (if_idx == 1) {
                                memcpy(eth_hdr->ether_dhost, MAC3,
                                       sizeof(ether_addr));
                            }
#endif
                        }
