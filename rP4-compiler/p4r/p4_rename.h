...
template <typename T> class P4Rename {
public:
    ...
    void add_names(const std::vector<T>& v,
...
template <typename T>
void P4Rename<T>::add_names(const std::vector<T>& v,
                            std::function<std::string(const T&)> f) {
    for (auto& item : v) {
...
void P4Rename<T>::add_name(const std::string& name, bool suffix) {
    if (auto x =
            std::find_if(std::begin(original_names), std::end(original_names),
                         [&](auto& _name) { return name == _name; });
...
template <typename T>
P4Rename<T>::P4Rename(const std::vector<T>& v,
                      std::function<std::string(const T&)> f) {
    add_names(v, f);
}
                ...
                for (int x = 0; x < 10; x++) { // omit infinite loop
                    ...
                    if (next != std::string::npos) {
                        ...
                        if (std::find_if(
                                ...
                                std::end(original_names), [&](auto& name) {
                                    return name != original_name &&
                                           name.size() >= test_text.size() &&
