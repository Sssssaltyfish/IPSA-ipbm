...
void extract_headers() {
    for (auto& header_json : j.at("headers")) {
        ...
        header.header_type_name =
            header_json.at("header_type").get<std::string>();
    ...
    for (auto& parser_json : j.at("parsers")) {
        ...
        for (auto& parse_state_json : parser_json.at("parse_states")) {
            ...
            for (auto& parse_op_json : parse_state_json.at("parser_ops")) {
                ...
                parser_op.parameter_name =
                    parse_op_json.at("parameters").at(0).at("value");
                parse_state.parser_ops.push_back(parser_op);
            }
            for (auto& transition_json : parse_state_json.at("transitions")) {
                Transition transition;
                transition.value =
                    transition_json.at("value").get<std::string>();
                transition.mask = safe_convert(transition_json.at("mask"));
                transition.next_state =
                    safe_convert(transition_json.at("next_state"));
                parse_state.transitions.push_back(transition);
            }
            for (auto& transition_key_json :
                 parse_state_json.at("transition_key")) {
...
P4Parameter extract_parameter(json j) {
    ...
    if (pp.type == "field") {
        ...
        if (nested.find("type") != std::end(nested)) {
            ...
            exp->left =
                (nested.at("left").is_null()
                     ? nullptr
                     : new P4Parameter(extract_parameter(nested.at("left"))));
            exp->right =
                (nested.at("right").is_null()
                     ? nullptr
                     : new P4Parameter(extract_parameter(nested.at("right"))));
    ...
    for (auto& action_json : j.at("actions")) {
        ...
        for (auto& primitive_json : action_json.at("primitives")) {
            ...
            for (auto& parameter_json : primitive_json.at("parameters")) {
                primitive.parameters.push_back(
                    extract_parameter(parameter_json));
            }
    ...
    for (auto& pipeline_json : j.at("pipelines")) {
        ...
        for (auto& table_json : pipeline_json.at("tables")) {
            ...
            table.default_entry.action_id =
                default_entry_json.at("action_id").get<int>();
            for (auto& action_data_json :
                 default_entry_json.at("action_data")) {
                table.default_entry.action_data.push_back(
                    action_data_json.get<std::string>());
            }
                ...
                for (auto& entry_json : table_json.at("entries")) {
                    ...
                    for (auto& match_key_json : entry_json.at("match_key")) {
                        ...
                        match_key.match_type =
                            match_key_json.at("match_type").get<std::string>();
                        match_key.key =
                            match_key_json.at("key").get<std::string>();
                        if (match_key.match_type == "lpm") {
                            match_key.prefix_length =
                                match_key_json.at("prefix_length").get<int>();
                        } else if (match_key.match_type == "ternary") {
                            match_key.mask =
                                match_key_json.at("mask").get<std::string>();
                        }
                        entry.match_key.push_back(match_key);
                    }
                    entry.action_id = entry_json.at("action_entry")
                                          .at("action_id")
                                          .get<int>();
                    for (auto& action_data_json :
                         entry_json.at("action_entry").at("action_data")) {
                        entry.action_data.push_back(
                            action_data_json.get<std::string>());
                    }
        ...
        for (auto& conditional_json : pipeline_json.at("conditionals")) {
            ...
            conditional.true_next =
                safe_convert(conditional_json.at("true_next"));
            conditional.false_next =
                safe_convert(conditional_json.at("false_next"));
            conditional.expression =
                extract_parameter(conditional_json.at("expression"));
