...
public:
    ...
    friend std::ostream& operator<<(std::ostream& out,
                                    P4Actions const& actions);
};

std::string P4Actions::translate_name(const std::string& name,
                                      int index) const {
    if (action_rename == nullptr) {
        action_rename =
            new P4Rename<P4Action>(*this, [](auto& act) { return act.name; });
    }
    ...
    for (auto& action : actions) {
        ...
        for (auto& p : action.primitives) {
            ...
            if (p.op == "assign") {
                ...
                p.parameters[1].output(out, &(action.runtime_data))
                    << ";" << std::endl;
            } else {
