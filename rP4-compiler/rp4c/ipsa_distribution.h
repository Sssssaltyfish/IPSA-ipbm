...
private:
    void topologicalSort(const std::map<int, std::vector<int>>& last_table,
                         const std::map<int, std::vector<int>>& next_table,
                         std::vector<int>& topo_sequence);
};

inline void IpsaDistribution::topologicalSort(
...
inline void IpsaDistribution::distributeParsers() {
    ...
    for (auto& [name, header] : processor_manager->header_manager->headers) {
        // std::cout << "header " << header.header_id << " = " << name <<
        // std::endl;
        header_level.insert({{header.header_id, {}}});
    }
    for (int level_id = 0;
         auto& level : processor_manager->level_manager->levels) {
        for (auto& entry : level.entries) {
            ...
            if (std::find(std::begin(temp), std::end(temp), level_id) ==
                std::end(temp)) {
    ...
    for (int i = 0; i < processor_manager->level_manager->levels.size(); i++) {
        std::vector<int> temp;
        for (auto& entry :
             processor_manager->level_manager->levels[i].entries) {
            if (entry.parser_sram_entry.accept_drop < 2) {
                continue;
            }
            int next_level_id = entry.parser_sram_entry.next_state;
            if (std::find(std::begin(temp), std::end(temp), next_level_id) ==
                std::end(temp)) {
    ...
    for (int level_id : level_topological) {
        ...
        for (int pred_id : level_last_table.at(level_id)) {
            for (int level : level_level.at(pred_id)) {
                if (std::find(std::begin(pred_levels), std::end(pred_levels),
                              level) == std::end(pred_levels)) {
    ...
    for (auto& stage : processor_manager->stage_manager->logical_stages) {
        ...
        for (auto& header : stage.def->parser.headers) {
            int header_id =
                processor_manager->header_manager->get_header(header.name)
                    ->header_id;
            for (int level_id : header_level[header_id]) {
                for (int level : level_level[level_id]) {
                    if (std::find(std::begin(temp), std::end(temp), level) ==
                        std::end(temp)) {
        ...
        proc_level.insert({{stage.stage_id, std::move(temp)}});
    }
    // std::cout << "proc_level = ";
    ...
    for (auto& stage : processor_manager->stage_manager->logical_stages) {
        ...
        for (auto [act, proc] : stage.action_proc) {
            ...
            if (std::find(std::begin(temp), std::end(temp), proc) ==
                std::end(temp)) {
    ...
    for (int proc_id : topo_sequence) {
        ...
        if (last_proc_ids.size() == 0) {
            // first processor
            std::copy(std::begin(level_ids), std::end(level_ids),
            ...
            for (int x : one_last_total) {
                if (last_proc_ids.size() == 1 ||
                    std::all_of(
                        std::next(std::begin(last_proc_ids), 1),
                        std::end(last_proc_ids), [&](auto& last_proc_id) {
                            auto& total_level = total_levels[last_proc_id];
                            return std::find(std::begin(total_level),
                                             std::end(total_level),
                                             x) != std::end(total_level);
                        })) {
                    total.push_back(x);
                }
            }
            for (int level_id : level_ids) {
                if (std::find(std::begin(total), std::end(total), level_id) ==
                    std::end(total)) {
                    temp.push_back(level_id);
    ...
    //     std::cout << x << " = " <<
    //     processor_manager->stage_manager->logical_stages[x].def->name << " ";
