...
public:
    ...
    Rp4MatchKey() {}
    Rp4MatchKey(std::vector<std::shared_ptr<Rp4Key>> _keys)
        : keys(std::move(_keys)) {}
    virtual std::string toString() const { return "match-key"; }
...
public:
    ...
    Rp4TableEntriesStmt() {}
    Rp4TableEntriesStmt(std::vector<Rp4MatchEntry> _entries)
        : entries(std::move(_entries)) {}
    virtual std::string toString() const { return "table-entries-stmt"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
...
public:
    ...
    Rp4DefaultActionStmt(std::string _action_name)
        : action_name(std::move(_action_name)) {}
...
public:
    ...
    Rp4TableKeyDef() {}
    Rp4TableKeyDef(std::vector<Rp4KeyEntry> _entries)
        : entries(std::move(_entries)) {}
    virtual std::string toString() const { return "table-key"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
        return mapped(entries);
    }
};
...
public:
    ...
    Rp4TableDef() {}
    Rp4TableDef(
        std::string _name, Rp4TableKeyDef _key_def,
        std::vector<std::shared_ptr<Rp4TableOptionalStmt>> _optional_stmts)
        : name(std::move(_name)), key_def(std::move(_key_def)),
          optional_stmts(std::move(_optional_stmts)) {}
    virtual std::string toString() const { return "table(" + name + ")"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
...
public:
    ...
    Rp4TablesDef() {}
    Rp4TablesDef(std::vector<Rp4TableDef> _tables)
        : tables(std::move(_tables)) {}
    virtual std::string toString() const { return "tables"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
