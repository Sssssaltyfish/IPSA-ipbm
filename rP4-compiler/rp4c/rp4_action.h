...
public:
    ...
    Rp4ActionDef() {}
    Rp4ActionDef(std::string _name, std::vector<Rp4FieldDef> _parameters,
                 std::vector<Rp4Expression> _expressions,
                 bool _is_noaction = false)
        : name(std::move(_name)), parameters(std::move(_parameters)),
          expressions(std::move(_expressions)), is_noaction(_is_noaction) {}
    virtual std::string toString() const { return "action(" + name + ")"; }
...
public:
    ...
    Rp4ActionsDef() {}
    Rp4ActionsDef(std::vector<Rp4ActionDef> _actions)
        : actions(std::move(_actions)) {}
    virtual std::string toString() const { return "actions"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
