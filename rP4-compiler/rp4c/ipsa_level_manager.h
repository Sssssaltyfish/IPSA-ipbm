...
public:
    ...
    IpsaParserTcamEntry(int _current_state, const Rp4Key* key)
        : current_state(_current_state), entry(key->get_key()),
          mask(key->get_mask()) {}
...
public:
    ...
    int accept_drop;
    IpsaParserSramEntry(int _header_id, int _header_length, int _next_state,
                        std::vector<IpsaHeaderField> _transition_fields,
                        int _accept_drop)
        : header_id(_header_id), header_length(_header_length),
          next_state(_next_state),
          transition_fields(std::move(_transition_fields)),
          accept_drop(_accept_drop) {}
    virtual std::shared_ptr<IpsaValue> toIpsaValue() const {
        std::map<std::string, std::shared_ptr<IpsaValue>> dst = {
...
public:
    ...
    IpsaLevelEntry(IpsaParserTcamEntry _parser_tcam_entry,
                   IpsaParserSramEntry _parser_sram_entry)
        : parser_tcam_entry(std::move(_parser_tcam_entry)),
          parser_sram_entry(std::move(_parser_sram_entry)) {}
    virtual std::shared_ptr<IpsaValue> toIpsaValue() const {
        std::map<std::string, std::shared_ptr<IpsaValue>> dst = {
...
inline void IpsaLevelManager::load(const Rp4Ast* ast) {
    auto& state_defs = ast->parser_def.states;
    global_level_id = 0;
    auto get_state = [&](std::string state_name) -> const Rp4StateDef* {
        if (auto state_def = std::find_if(
                std::begin(state_defs), std::end(state_defs),
                [&](auto& state) { return state.name == state_name; });
    ...
    //         if (trans->isDirect()) {
    //             default_entry =
    //             &(std::static_pointer_cast<Rp4DirectTransition>(trans)->entry);
    //         } else if (trans->isSelect()) {
    //             auto& entries =
    //             std::static_pointer_cast<Rp4SelectTransition>(trans)->entries;
    //             if (auto de = std::find_if(std::begin(entries),
    //             std::end(entries), [](auto& entry) {
    ...
    // };
    auto get_select =
        [&](const Rp4StateDef* state_def) -> std::vector<IpsaHeaderField> {
        if (state_def == nullptr) {
            return {};
        }
        auto trans = state_def->transition;
        ...
        auto& targets =
            std::static_pointer_cast<Rp4SelectTransition>(trans)->targets;
    ...
    auto build_entries = [&](IpsaLevel& level, int state,
                             const std::vector<Rp4TransitionEntry>& entries) {
        for (auto& entry : entries) {
            if (entry.accept_drop == 2) {
                auto next_state = get_state(entry.next);
                auto m =
                    header_manager->get_header(&(next_state->extract.target));
                level.add(IpsaLevelEntry(
                    IpsaParserTcamEntry(state, entry.key.get()),
                    IpsaParserSramEntry(m->header_id, m->offset,
    ...
    if (state_defs.size() > 0) {
        ...
        levels[0].add(
            IpsaLevelEntry(IpsaParserTcamEntry(0, 0, 0),
                           IpsaParserSramEntry(m->header_id, m->offset,
                                               states[first_state->name],
        ...
        for (auto& state_def : state_defs) {
            if (auto x = states.find(state_def.name); x != std::end(states)) {
                ...
                if (trans->isDirect()) {
                    build_entries(
                        level, x->second,
                        {std::static_pointer_cast<Rp4DirectTransition>(trans)
                             ->entry});
                } else if (trans->isSelect()) {
                    build_entries(
                        level, x->second,
                        std::static_pointer_cast<Rp4SelectTransition>(trans)
                            ->entries);
                }
            }
        }
