...
public:
    ...
    IpsaAssign() {}
    IpsaAssign(const IpsaHeaderField* _lvalue,
               std::shared_ptr<IpsaOperand> _rvalue)
...
public:
    ...
    IpsaAction(int _id, const Rp4ActionDef* _action_def)
        : id(_id), action_def(_action_def) {}
    virtual std::shared_ptr<IpsaValue> toIpsaValue() const {
        std::map<std::string, std::shared_ptr<IpsaValue>> dst = {
            {"id", makeValue(id)},
...
public:
    ...
    IpsaActionManager(IpsaHeaderManager* _header_manager)
...
bool IpsaActionManager::concatAction(int id, const IpsaAction* next_action) {
    auto can_concat = [](const std::vector<IpsaAssign>& assign1,
                         const std::vector<IpsaAssign>& assign2) {
        for (auto& a1 : assign1) {
            for (auto& a2 : assign2) {
    ...
    for (auto& [_, x] : actions) {
        if (x.id == id) {
            if (can_concat(x.primitives, next_action->primitives)) {
                std::copy(std::begin(next_action->primitives),
                          std::end(next_action->primitives),
...
void IpsaActionManager::addAction(const Rp4ActionDef* action_def) {
    if (actions.find(action_def->name) == std::end(actions)) {
        ...
        actions.insert({{action_def->name, std::move(action)}});
    }
}

inline void IpsaActionManager::load(const Rp4Ast* ast) {
    global_action_id = 0;
