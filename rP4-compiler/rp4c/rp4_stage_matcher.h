...
public:
    ...
    bool to_none;
    Rp4SwitchStageStmt() {}
    Rp4SwitchStageStmt(std::string _name, bool _to_none = false)
        : name(std::move(_name)), to_none(_to_none) {}
    virtual std::string toString() const {
...
public:
    ...
    Rp4SwitchEntry() {}
    Rp4SwitchEntry(std::shared_ptr<Rp4Key> _key,
                   std::shared_ptr<Rp4SwitchValue> _value)
        : key(std::move(_key)), value(std::move(_value)) {}
    virtual std::string toString() const { return "switch-entry"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
        return {dynamic_cast<const Rp4TreeNode*>(key.get()),
...
public:
    ...
    Rp4StageMatcher() {}
    Rp4StageMatcher(std::vector<std::shared_ptr<Rp4Operation>> _ops,
                    std::vector<Rp4SwitchEntry> _switch_entries)
        : ops(std::move(_ops)), switch_entries(std::move(_switch_entries)) {}
    virtual std::string toString() const { return "matcher-in-stage"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
        auto dst = mapped(ops);
