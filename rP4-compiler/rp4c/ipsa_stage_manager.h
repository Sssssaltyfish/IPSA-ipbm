...
inline void IpsaStageManager::load(const Rp4Ast* ast) {
    global_stage_id = 0;
...
inline void IpsaStageManager::add_stage(const Rp4StageDef* stage_def,
                                        bool in_ingress) {
    if (logical_stages_map.find(stage_def->name) ==
        std::end(logical_stages_map)) {
        int x = logical_stages.size();
        logical_stages.push_back(
            IpsaStage(stage_def, global_stage_id++, in_ingress));
        logical_stages_map.insert({{stage_def->name, x}});
    }
}

inline const IpsaStage* IpsaStageManager::lookup(std::string stage_name) const {
    if (auto x = logical_stages_map.find(stage_name);
        x != std::end(logical_stages_map)) {
...
inline const IpsaStage* IpsaStageManager::lookup(int stage_id) const {
