    ...
    virtual std::shared_ptr<IpsaValue> toIpsaValue() const {
        ...
        for (int i = 0; i < ipsa_configuration::PROC_COUNT; i++) {
            dst.insert({{"processor_" + std::to_string(i),
                         processors[i].get() == nullptr
                             ? nullptr
                             : processors[i]->toIpsaValue()}});
        }
        return makeValue(dst);
    }
};
...
public:
    ...
    Ipsa ipsa;
    IpsaBuilder()
        : header_manager(), level_manager(&header_manager),
          action_manager(&header_manager),
          table_manager(&header_manager, &action_manager), stage_manager(),
          gateway_manager(&header_manager, &stage_manager, &table_manager),
          processor_manager(&stage_manager, &header_manager, &level_manager,
                            &action_manager, &table_manager),
...
inline void IpsaBuilder::allocateProcessors() {
    for (int i = 0; i < ipsa_configuration::PROC_COUNT; i++) {
    ...
    for (int i = 0; i < ipsa_configuration::PROC_COUNT; i++) {
        ...
        if (auto it = memory.physical_proc_id.find(i);
            ...
            for (int j = 0; j < parser_levels.size();
                 j += ipsa_configuration::MAX_LEVEL) {
                int target_stage = i;
                if (j + ipsa_configuration::MAX_LEVEL <
                    parser_levels.size()) { // not the last
                    for (int k = 0; k < ipsa_configuration::PROC_COUNT;
                         k++) { // allocate a new processor
                        if (ipsa.processors[k].get() == nullptr &&
                            memory.physical_proc_id[k] < 0) {
                            ipsa.processors[k] =
                                std::make_shared<IpsaProcessor>(
                                    extra_processor_id++);
                            target_stage = k;
                            break;
                        }
                    ...
                    if (target_stage == i) {
                        std::cout << "error no processors can be allcoated"
                                  << std::endl;
                    }
                }
                auto& parser = ipsa.processors[target_stage]->parser;
                for (int k = 0; k < ipsa_configuration::MAX_LEVEL &&
                                j + k < parser_levels.size();
                     k++) {
                ...
                if (target_stage != i) {
                    ...
                    int next_stage_id = x;
                    if (j + ipsa_configuration::MAX_LEVEL * 2 <
                        parser_levels.size()) {
                        next_stage_id = extra_processor_id;
                    }
                    // may cause memory leak, set the mark "user_gateway" to
                    // avoid it
                    ipsa.processors[target_stage]->user_gateway = true;
                    auto gateway = ipsa.processors[target_stage]->gateway =
                        new IpsaGateway();
                    gateway->next_table.default_entry =
                        std::make_shared<IpsaGatewayStageEntry>(next_stage_id);
        ...
        if (int i = proc_cluster[stage_id]; i >= 0) {
            if (cluster_proc[i].size() <
                ...
                cluster_space[i] = {
                    cluster_space[i].first - proc_space[stage_id].first,
                    cluster_space[i].second - proc_space[stage_id].second};
                int target = i * ipsa_configuration::CLUSTER_PROC_COUNT +
                             cluster_proc[i].size();
