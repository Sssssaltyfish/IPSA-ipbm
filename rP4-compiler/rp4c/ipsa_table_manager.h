...
public:
    ...
    IpsaTableManager(IpsaHeaderManager* _header_manager,
    ...
    void setMatcherId(int table_id, int proc_id, int matcher_id,
        ...
        if (table.table_id == table_id) {
            ...
            for (auto [action, proc] : action_proc) {
                table.action_to_proc.push_back(
                    IpsaActionProcPair(action, proc));
            }
            break;
        }
    }
...
inline void IpsaTableManager::load(const Rp4Ast* ast) {
    global_table_id = 0;
    for (auto& table_def : ast->tables_def.tables) {
        ...
        for (auto x : table_def.optional_stmts) {
            if (x->isDefault()) {
                auto& y = std::static_pointer_cast<Rp4DefaultActionStmt>(x)
                              ->action_name;
        ...
        if (key_def_entries.size() == 0) {
            ...
            if (key_def_entries.size() == 1) {
                ...
                if (std::all_of(std::begin(key_def_entries),
                                std::end(key_def_entries), [](auto& entry) {
            ...
            int mem_width = table.key_memory.type == MEM_SRAM
                                ? ipsa_configuration::SRAM_WIDTH
                                : ipsa_configuration::TCAM_WIDTH;
            int mem_depth = table.key_memory.type == MEM_SRAM
                                ? ipsa_configuration::SRAM_DEPTH
                                : ipsa_configuration::TCAM_DEPTH;
            table.key_memory.width =
                (table_key_width + (mem_width - 1)) / mem_width;
            int depth = 1;
            for (auto x : table_def.optional_stmts) {
                if (x->isSize()) {
            ...
            for (auto x : table_def.optional_stmts) {
                if (x->isWidth()) {
                    width = std::static_pointer_cast<Rp4TableValueWidthStmt>(x)
                                ->width;
