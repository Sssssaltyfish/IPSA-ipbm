...
enum IpsaFieldType { FT_FIELD, FT_VALID, FT_HIT, FT_MISS };

static inline std::string to_string(IpsaFieldType op) {
    static const std::map<IpsaFieldType, std::string> m = {{FT_FIELD, "FIELD"},
                                                           {FT_VALID, "VALID"},
                                                           {FT_HIT, "HIT"},
                                                           {FT_MISS, "MISS"}};
    return m.at(op);
...
public:
    ...
    int field_length;
    IpsaHeaderField(IpsaFieldType _type, int _header_id = 0,
                    int _field_internal_offset = 0, int _field_length = 0)
        : type(_type), header_id(_header_id),
          field_internal_offset(_field_internal_offset),
    ...
    for (offset = 0; auto& field : header_def->fields) {
        if (auto x = field.type; x->isBitType()) {
            ...
            fields.insert({{field.name,
                            IpsaHeaderField(FT_FIELD, id, offset, y->length)}});
            offset += y->length;
        } else {
...
IpsaHeader::IpsaHeader(const Rp4StructDef* struct_def, int id)
    ...
    for (offset = 0; auto& field : struct_def->fields) {
        if (auto x = field.type; x->isBitType()) {
            ...
            std::cout << "error metadata: " << struct_def->struct_name
                      << std::endl;
        }
    }
}
...
public:
    ...
    const IpsaHeaderField* lookup(std::string header_name,
...
const IpsaHeader* IpsaHeaderManager::get_header(const Rp4Member* member) const {
    if (member->instance_name == this->header_name) {
        ...
        std::cout << "error wrong headers name: " << member->instance_name
                  << std::endl;
        return nullptr;
    }
}
...
IpsaHeaderManager::lookup(std::shared_ptr<Rp4LValue> lvalue) const {
    ...
    if (lvalue->isMeta()) {
        ...
        if (header.instance_name == this->header_name) {
            ...
            std::cout << "error wrong headers name: " << header.instance_name
                      << std::endl;
            return nullptr;
        }
    } else {
        ...
        if (struct_def.is_header) {
            ...
            for (auto& field_def : struct_def.fields) {
                if (auto x = field_def.type; x->isNamedType()) {
                    ...
                    if (auto header_def = std::find_if(
                            std::begin(ast->header_defs),
                            std::end(ast->header_defs),
                            [&](auto& h) { return h.name == y->name; });
                        header_def != std::end(ast->header_defs)) {
                        addHeader(header_def.base(), field_def.name);
                    } else {
                        std::cout << "error no header type: " << y->name
                                  << std::endl;
                    }
                } else {
                    std::cout << "error bit type in headers: " << field_def.name
                              << std::endl;
                }
            }
        } else {
    ...
    for (auto& [name, header] : headers) {
        if (!header.is_header && !header.is_standard_metadata) {
            ...
            metadata.entries.push_back(
                IpsaMetadataEntry(header.header_id, offset, length));
            offset += length;
        }
    }
}