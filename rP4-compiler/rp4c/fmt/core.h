...
public:
    ...
    FMT_CONSTEXPR_CHAR_TRAITS bool
...
template <typename Char>
FMT_CONSTEXPR void
basic_format_parse_context<Char>::check_dynamic_spec(int arg_id) {
    if (detail::is_constant_evaluated() &&
        (!FMT_GCC_VERSION || FMT_GCC_VERSION >= 1200)) {
        using context = detail::compile_parse_context<Char>;
...
template <typename Char>
FMT_CONSTEXPR FMT_INLINE auto
parse_format_specs(const Char* begin, const Char* end,
                   dynamic_format_specs<Char>& specs,
                   basic_format_parse_context<Char>& ctx, type arg_type)
    -> const Char* {
    auto c = '\0';
    if (end - begin > 1) {
    ...
    for (;;) {
        ...
        case '{':
            ...
            begin = parse_dynamic_spec(begin, end, specs.width, specs.width_ref,
                                       ctx);
            break;
        case '.':
            enter_state(state::precision,
                        in(arg_type, float_set | string_set | cstring_set));
            begin = parse_precision(begin, end, specs.precision,
...
private:
    ...
    // In the future basic_format_parse_context will replace
    // compile_parse_context here and will use is_constant_evaluated and
    // downcasting to access the data needed for compile-time checks:
...
public:
    ...
    FMT_CONSTEXPR auto parse(ParseContext& ctx) -> const Char* {
        ...
        auto end = detail::parse_format_specs(ctx.begin(), ctx.end(), specs_,
                                              ctx, type);
