...
public:
    ...
    virtual std::string toString() const { return "extract"; }
...
public:
    ...
    Rp4SelectTransition() {}
    Rp4SelectTransition(std::vector<Rp4Field> _targets,
                        std::vector<Rp4TransitionEntry> _entries)
        : targets(std::move(_targets)), entries(std::move(_entries)) {}
    virtual bool isSelect() const { return true; }
    virtual std::string toString() const { return "transition-select"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
...
public:
    ...
    virtual std::string toString() const { return "transition-direct"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
...
public:
    ...
    Rp4StateDef() {}
    Rp4StateDef(std::string _name, Rp4Extract _extract,
                std::shared_ptr<Rp4Transition> _transition)
        : name(std::move(_name)), extract(std::move(_extract)),
          transition(std::move(_transition)) {}
    virtual std::string toString() const { return "state-def(" + name + ")"; }
    virtual std::vector<const Rp4TreeNode*> children() const {
        return {dynamic_cast<const Rp4TreeNode*>(&extract),
    ...
    int get_accept_drop() const {
        ...
        if (transition->isSelect()) {
            auto& entries =
                std::static_pointer_cast<Rp4SelectTransition>(transition)
                    ->entries;
            if (std::all_of(std::begin(entries), std::end(entries),
                            is_accept)) {
                return 0;
            } else if (std::all_of(std::begin(entries), std::end(entries),
                                   is_drop)) {
                return 1;
            }
        } else if (transition->isDirect()) {
            auto& entry =
                std::static_pointer_cast<Rp4DirectTransition>(transition)
                    ->entry;
            if (is_accept(entry)) {
                return 0;
            } else {
